(function () {
        ...
        if (podeGer) {
          html +=
            '<div class="feit-ano-item">' +
              '<button class="feit-ano-btn feit-ano-liberado" onclick="window._feitAbrirModalAno(' + (i+1) + ')" title="' + ano + '">' + label + '</button>' +
              '<button class="feit-ano-toggle" onclick="window._feitToggleAno(\'' + uid + '\',' + (i+1) + ',true)" title="Bloquear ano"><i class="fa-solid fa-lock-open"></i></button>' +
            '</div>';
        } else {
          html += '<button class="feit-ano-btn feit-ano-liberado" onclick="window._feitAbrirModalAno(' + (i+1) + ')" title="' + ano + '">' + label + '</button>';
        }
      } else if (podeGer) {
        html +=
          '<div class="feit-ano-item">' +
            '<button class="feit-ano-btn feit-ano-bloqueado" onclick="window._feitAbrirModalAno(' + (i+1) + ')" title="' + ano + '">' + label + '</button>' +
            '<button class="feit-ano-toggle" onclick="window._feitToggleAno(\'' + uid + '\',' + (i+1) + ',false)" title="Liberar ano"><i class="fa-solid fa-lock"></i></button>' +
          '</div>';
      }
    });
    wrap.innerHTML = html;
  ...
  window._feitAbrirModalAno = function(num) {
    ...
    var itens = Object.keys(catalogo).filter(function(slug) {
      return (catalogo[slug].dificuldade || '').toLowerCase() === anoLabel.toLowerCase();
    }).sort(function(a, b) {
    ...
    modal.innerHTML =
      '<div class="feit-modal-box">' +
        ...
        '<div class="feit-modal-corpo">' +
          (itens.length
            ? itens.map(function(slug) { return montarCard(slug, catalogo[slug], false); }).join('')
            : '<div class="feit-vazio">Nenhum feitiço cadastrado para este ano.</div>') +
        '</div>' +
      '</div>';
  ...
  window._feitToggleAno = function(uid, num, liberadoAtual) {
    ...
    var path = '/feiticos-anos/' + fkeyF(uid) + '/' + num;
    var op = novoValor ? fpPut(path, true) : fpDel(path);
    op.then(function() {
