...
function renderMural(match){
  ...
  var entradas = fixas.concat(normais);
...
/* ============================================================
   MOTOR SEQUENCIAL — EM CONSTRUÇÃO, AINDA NÃO LIGADO AO JOGO REAL
   ============================================================
   Fatia 1a: ações que NÃO dependem de nenhuma outra ação desta
   mesma fase (só olham pra estado permanente: rastreio, debuff).
   Isso substituirá parte da resolução em lote de resolverFase(),
   quando todas as fatias estiverem prontas e integradas juntas.
   ============================================================ */
    ...
    case 'sabotar': {
      return consumirBonusSituacionalSeq(pid, faseNum, time, pos).then(function(bonusAcumulado){
        ...
        if(vencedor(rAtk, rDef)){
          ...
          log(slotAtor.nome + ' sabotou ' + advAph.nome + '. Rastreamento zerado.', { afeta: [advT(time) + '_apanhador'] });
        }else{
          log(slotAtor.nome + ' tentou sabotar, mas falhou.');
        }
        return finalizar();
      });
    }
    ...
    case 'carregar': {
      return consumirBonusSituacionalSeq(pid, faseNum, time, pos).then(function(bonusAcumulado){
        ...
        if(vencedor(rAtk2, rDef2)){
          ...
          log(slotAtor.nome + ' derrubou ' + advAph2.nome + '! Rastreamento zerado, ele não age nesta fase.', { afeta: [advT(time) + '_apanhador'] });
          if(advAph2.uid && advAph2.uid !== 'npc') savesExtra.push(deduzirHp(advAph2.uid, QUAD_HP_DANO));
        }else{
          log(slotAtor.nome + ' tentou derrubar ' + advAph2.nome + ', mas foi esquivado.');
        }
        return finalizar();
      });
    }
...
/* ============================================================
   Fatia 1b: ações "vs alvo", mas que rolam contra os atributos
   BASE do alvo (não dependem da ação que ele escolheu, só de
   estado já resolvido antes nesta fase — protegido, fintado etc,
   que ainda ficam 0/ausentes até a Fatia 1c existir).
   ============================================================ */
  ...
  switch(ac.acao){
    ...
    case 'finta_passe': {
      ...
      if(vencedor(rAtkFP, rDefFP)){
        ...
        log(slotAtor.nome + ' enganou o Goleiro com uma Finta de Passe. -3 na defesa dele nesta fase.', { afeta: [advTime + '_goleiro'] });
      }else{
        log(slotAtor.nome + ' tentou uma Finta de Passe, mas foi lido.');
      }
      return finalizar();
    }
    ...
    case 'antecipar': {
      ...
      if(vencedor(rDefAnt, rAtkAnt)){
        ...
        log(slotAtor.nome + ' antecipou o jogo. +5 na defesa desta fase.');
      }else{
        log(slotAtor.nome + ' tentou antecipar, mas não conseguiu.');
      }
    ...
    case 'mergulho': {
      ...
      if(vencedor(rAtkMer, rDefMer)){
        ...
        log(slotAtor.nome + ' fez um Mergulho impressionante. +4 no arremesso.');
      }else{
        savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/expostos/' + time + '_' + pos, true));
        log(slotAtor.nome + ' tentou mergulhar mas foi bloqueado. Ficou exposto.');
      }
      return finalizar();
    }
    ...
    case 'fintar': {
      ...
      if(vencedor(rAtkFin, rDefFin)){
        ...
        savesExtra.push(somarMvpSeq(pid, time + '_' + pos, QUAD_MVP_CONTROLE));
        log(slotAtor.nome + ' executou uma finta perfeita. Imune ao Balaço esta fase.');
      }else{
        log(slotAtor.nome + ' tentou fintar, mas o Batedor previu.');
      }
      return finalizar();
    }
    ...
    case 'derrubar_artilheiro': {
      ...
      if(vencedor(rAtkDA, rDefDA)){
        ...
        log(slotAtor.nome + ' derrubou ' + alsDA.nome + ' no ar! Não vai arremessar esta fase. -' + QUAD_HP_DANO + ' HP.', { afeta: [advTime + '_' + ac.alvo] });
        if(alsDA.uid && alsDA.uid !== 'npc') savesExtra.push(deduzirHp(alsDA.uid, QUAD_HP_DANO));
      }else{
        log(slotAtor.nome + ' tentou derrubar ' + alsDA.nome + ', mas errou.');
      }
    ...
    case 'interceptar': {
      ...
      if(vencedor(rAtkInt, rDefInt)){
        ...
        log(slotAtor.nome + ' interceptou o passe adversário! Artilheiros adversários perdem o bônus de Passar a partir de agora.');
      }else{
        log(slotAtor.nome + ' tentou interceptar, mas falhou.');
      }
      return finalizar();
    }
    ...
    case 'blitz': {
      ...
      if(vencedorOuIgual(rDefBli, rAtkBli)){
        ...
        log(slotAtor.nome + ' executou o Blitz com sucesso! Interceptou a Goles.');
      }else{
        savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/blitzResult/' + time, 'fail'));
        log(slotAtor.nome + ' tentou o Blitz e falhou! Os aros ficaram desprotegidos.');
      }
      return finalizar().then(function(resultado){
        return resolverPendentesContraGoleiro(pid, match, faseNum, time).then(function(){ return resultado; });
      });
...
/* ============================================================
   Fatia 1c: as ações mais entrelaçadas — Proteger, Lançar Balaço,
   Derrubar, Pressão Aérea, Defender e Arremessar (a única que
   genuinamente precisa esperar o Goleiro agir, se ele ainda não
   agiu nesta fase — o mecanismo de "pendente" combinado).
   ============================================================ */
...
// resolve UM arremesso específico, assumindo que os dados do goleiro
// (antecipeBonus / blitzResult / fintaPasseDebuff / a própria ação dele)
// já estão disponíveis — ou porque ele já agiu antes, ou porque
// acabou de agir agora e isso está liberando os pendentes contra ele.
  ...
  switch(ac.acao){
    ...
    case 'lancar_balaco': {
      ...
      if(vencedor(rAtkLB, rDefLB)){
        ...
        log(slotAtor.nome + ' acertou o Balaço em ' + alsLB.nome + '! -2 em ' + (MAIN_ATTR[ac.alvo] || 'agilidade') + ' e -' + QUAD_HP_DANO + ' HP.', { afeta: [alvKeyLB] });
        if(alsLB.uid && alsLB.uid !== 'npc') savesExtra.push(deduzirHp(alsLB.uid, QUAD_HP_DANO));
      }else{
        log(slotAtor.nome + ' lançou o Balaço em ' + alsLB.nome + ', mas errou.');
      }
      return finalizar();
    }
    ...
    case 'derrubar': {
      ...
      if(vencedor(rAtkD, rDefD)){
        savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/derrubados/' + advTime + '_' + ac.alvo, true));
        savesExtra.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_DEBUFF));
        log(slotAtor.nome + ' derrubou ' + alsD.nome + ' em colisão! Não age nesta fase. -' + QUAD_HP_DANO + ' HP.', { afeta: [advTime + '_' + ac.alvo] });
        if(alsD.uid && alsD.uid !== 'npc') savesExtra.push(deduzirHp(alsD.uid, QUAD_HP_DANO));
      }else{
        log(slotAtor.nome + ' tentou derrubar ' + alsD.nome + ', mas foi evitado.');
      }
      return finalizar();
    }
    ...
    case 'pressao_aerea': {
      ...
      if(vencedor(rAtkPA, rDefPA)){
        ...
        savesExtra.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_DEBUFF));
        log(slotAtor.nome + ' intimidou ' + alsPA.nome + ' no ar. -2 no próximo roll dele.', { afeta: [advTime + '_' + ac.alvo] });
      }else{
        log(slotAtor.nome + ' tentou intimidar ' + alsPA.nome + ', sem efeito.');
      }
      return finalizar();
    }

    case 'defender': {
      // sem rolagem própria — só "libera" a informação de que o goleiro
      // escolheu defender, e resolve qualquer arremesso que já estava pendente
      return finalizar().then(function(resultado){
        return resolverPendentesContraGoleiro(pid, match, faseNum, time).then(function(){ return resultado; });
      });
    }
    ...
    case 'arremessar': {
      ...
      if(!goleiroJaResolveu){
        // pendente: o goleiro ainda não agiu nesta fase — fica esperando
        ...
        // não marca resolvida ainda — fica pendente de verdade
...
/* ============================================================
   Fatia/Etapa 4: Muralha (Antecipar+Interceptar) e Cerco
   (Blitz+Interceptar) — combos "de sincronia com risco": o
   primeiro dos dois (Goleiro ou Batedor) fica pendente aguardando
   o parceiro; se o parceiro completar, rola combinado; se o
   parceiro fizer outra coisa (ou nunca chegar a completar),
   libera pra resolver sozinho.
   ============================================================ */
...
function resolverComboCombinado(pid, match, faseNum, time, comboPendente, posCompletou, acCompletou){
  ...
  if(tipoCombo === 'muralha'){
    ...
    if(sucesso){
      ...
      log('Combo: Muralha. ' + gs.nome + ' e ' + bs.nome + ' formam uma defesa perfeita.', { combo:true, nome:'Muralha' });
    }else{
      log(gs.nome + ' e ' + bs.nome + ' tentaram a Muralha, mas falharam.');
    }
  }else{
    ...
    if(sucesso){
      ...
      saves.push(somarMvpSeq(pid, time + '_goleiro', QUAD_MVP_COMBO));
      saves.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_COMBO));
      log('Combo: Cerco. ' + gs.nome + ' e ' + bs.nome + ' encurralam ' + advArt.nome + '. Debuff permanente aplicado.', { combo:true, nome:'Cerco' });
    }else{
      saves.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/blitzResult/' + time, 'fail'));
      log(gs.nome + ' e ' + bs.nome + ' tentaram o Cerco, mas falharam.');
    }
  }
...
/* ============================================================
   Integração: fechamento de fase sequencial (substitui o
   resolverFase/fecharFase em lote quando a fila termina)
   ============================================================ */
