...
function renderMural(match){
  ...
  var entradas = normais.concat(fixas);
...
var elJog = document.getElementById('q3jogadores');
  elJog.innerHTML = '';
  ['A','B'].forEach(function(t){
    var coresCasaTime = corDaCasaPorNome(match.times[t].nome);
    POSICOES.forEach(function(pos){
      var slot = match.times[t].slots[pos];
      if(!slot) return;
      ...
      div.dataset.time = t;
      div.dataset.pos = pos;
      if(coresCasaTime) div.style.setProperty('--casa-stroke', coresCasaTime.stroke);
...
function renderLogAbas(idTabs, idConteudo, fasesObj, mapaCores){
  var numeros = Object.keys(fasesObj || {}).map(Number).sort(function(a, b){ return a - b; });
...
function mapaCoresNomesJogadores(match){
  var mapa = {};
  ['A','B'].forEach(function(t){
    var cores = corDaCasaPorNome(match.times[t].nome);
    if(!cores) return;
    POSICOES.forEach(function(pos){
      var slot = match.times[t].slots[pos];
      if(slot && slot.nome) mapa[slot.nome] = cores.texto;
    });
  });
  return mapa;
}
