...
function iniciarJogo(match){
  ...
  document.getElementById('q3muraltitulo').onclick = function(){
    var conteudo = document.getElementById('q3muralconteudo');
    var icone = document.getElementById('q3muraltoggleicone');
    conteudo.hidden = !conteudo.hidden;
    icone.classList.toggle('q3muralrecolhido', conteudo.hidden);
  };
...
function resolverArremessoUnico(pid, match, faseNum, time, pos){
  ...
  if(vencedor(rAtk, rDef)){
    ...
    log(ast.nome + ' arremessou a Goles nos aros. GOOOOL! +' + pts + ' para ' + match.times[time].nome + '.', { afeta: [golvT + '_goleiro'] });
  }else{
    log(ast.nome + ' arremessou, mas ' + gs.nome + ' defendeu.', { afeta: [golvT + '_goleiro'] });
  }
  return finalizar();
}

// depois que o Goleiro finalmente resolve a própria ação (qualquer
// uma), essa função libera qualquer Arremesso que estava esperando ele
  ...
  return fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/pendentes').then(function(pendentes){
    ...
    return chaves.reduce(function(p, chave){
      return p.then(function(){
        ...
        return resolverArremessoUnico(pid, match, faseNum, timeAtacante, posAtacante);
      });
    }, Promise.resolve());
  });
}
...
// parceiro não completou o combo (fez outra coisa) — libera o que
// estava pendente pra resolver como ação solo normal
...
// dispatcher único de verdade — é isso que a integração final vai chamar
...
/* ============================================================
   Etapa 5: "Log jogador" — filtra o log da fase pra mostrar só
   o que afeta o próprio jogador (aliados ou adversários), com
   destaque maior pro aviso de combo pendente esperando confirmação.
   ============================================================ */

/* ============================================================
   Etapa 6: log em abas por fase (ao vivo e histórico) — aba do
   Pomo em dourado, ações novas sempre no topo, e cuidado pra não
   resetar o scroll nem reconstruir a barra de abas a cada poll.
   ============================================================ */

var logAbasEstado = {};

function renderLogAbas(idTabs, idConteudo, fasesObj){
  ...
  function renderConteudoAba(){
    ...
    elConteudo.innerHTML = logFase.slice().reverse().map(function(entrada){
      ...
      return '<div class="' + cls + '">' + escaparHtml(entrada.text || '') + '</div>';
    }).join('');
...
function resolverApanhadorEPomo(match, acoes, faseNum, combosRef, evento, lideranca){
  ...
  ['A','B'].forEach(function(t){
    ...
    if(!ac || !ac.alvo) return;

    if(ac.acao === 'inspirar'){
      log(sl2.nome + ' inspirou ' + sl(t, ac.alvo).nome + '. +2 de bônus na próxima ação dele.');
    }else if(ac.acao === 'provocar'){
      var alvoProvocado = sl(advT(t), ac.alvo);
      novosDebuffsApanhador[advT(t) + '_' + ac.alvo] = { atributo: MAIN_ATTR[ac.alvo] || 'agilidade', valor:-2, fases:1 };
      log(sl2.nome + ' provocou ' + alvoProvocado.nome + '. -2 no atributo principal por 1 fase.');
    }
  });
  ...
  ['A','B'].forEach(function(t){
    ...
    if(ac.acao === 'rastrear' && !distracoes[t]){
      ...
      var bonusInspiradoSab = bonusInspirado(acoes, t, 'apanhador');
      var advAph = sl(advT(t), 'apanhador');
      var rAtk = rolar(sl2, 'agilidade', 'determinacao', { match:match, time:t, pos:'apanhador', tipoVassoura:'manobrabilidade', especialidade:'pomo', acaoObj:ac, evento:evento, situacional:bonusMotivarSab + bonusInspiradoSab });
      ...
      var bonusInspiradoCar = bonusInspirado(acoes, t, 'apanhador');
      var advAph2 = sl(advT(t), 'apanhador');
      var rAtk2 = rolar(sl2, 'determinacao', 'agilidade', { match:match, time:t, pos:'apanhador', tipoVassoura:'velocidade', especialidade:'pomo', acaoObj:ac, evento:evento, situacional:bonusInspiradoCar });
  ...
  if(!match.pomo_capturado){
    ['A','B'].forEach(function(t){
      ...
      var advAph = sl(advT(t), 'apanhador');
      var bonusInspiradoMerg = bonusInspirado(acoes, t, 'apanhador');
      var rMeu = rolarPomo(sl2, { match:match, time:t, pos:'apanhador', acaoObj:ac, rastreio: rastreioAtual(t), evento:evento, situacional:bonusInspiradoMerg });
...
if(!match.pomo_capturado && !pomoBonusNovo && faseNum >= (match.pomo_fase || QUAD_POMO_MIN)){
  ...
  if(pomoBonusNovo && logArr.length){
    logArr[logArr.length - 1].pomo = true;
    logArr[logArr.length - 1].nome = 'Pomo Capturado';
  }
  ...
  return Promise.all(promessasHp).then(function(){
    return {
      ...
      pontosMvp: pontosMvp,
      novosDebuffs: novosDebuffsApanhador
    };
  });
}
...
function fecharFase(match, faseNum, r1, r2, evento){
  ...
  var todosNovosDebuffs = Object.assign({}, r1.novosDebuffs, r2.novosDebuffs);

  Object.keys(todosNovosDebuffs).forEach(function(key){
    var parts = key.split('_'); var t = parts[0], pos = parts.slice(1).join('_');
    saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + t + '/slots/' + pos, { debuff: todosNovosDebuffs[key] }));
  });
  ...
  ['A','B'].forEach(function(t){
    POSICOES.forEach(function(pos){
      var slot = match.times[t].slots[pos];
      var key = t + '_' + pos;
      if(slot && slot.debuff && !todosNovosDebuffs.hasOwnProperty(key)){
        var restante = slot.debuff.fases - 1;
        saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + t + '/slots/' + pos, {
  ...
  ['A','B'].forEach(function(t){
    var acAp = (match.fases[faseNum].acoes || {})[t + '_apanhador'];
    if(!acAp) return;
    if(QUAD_APANHADOR_ACOES_CACA.indexOf(acAp.acao) === -1) return;
    var slotAp = match.times[t].slots.apanhador;
    if(!slotAp) return;
    var novoCacaUsados = (slotAp.cacaUsados || 0) + 1;
    saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + t + '/slots/apanhador', { cacaUsados: novoCacaUsados }));
  });
...
function resolverArtGolBat(match, acoes, fase, bonusAtivo, combos, evento, lideranca){
  ...
  function opcoesRoll(t, pos, tipoVassoura, especialidade, acaoObj, situacional){
    ...
    var bonusInsp = bonusInspirado(acoes, t, pos);
    return { match:match, time:t, pos:pos, tipoVassoura:tipoVassoura, especialidade:especialidade, acaoObj:acaoObj, situacional:(situacional||0) + bonusMotivar + bonusInsp, evento:evento };
  }
