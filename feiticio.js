(function () {
  ...
  function bindSeletor(uid, aprendidos, catalogo) {
    ...
    function renderOpcoes(filtro) {
      var slugsTodos = Object.keys(catalogo).sort();
      var html = '';
      slugsTodos.forEach(function(slug) {
  ...
  function renderFeiticos(uid, aprendidos, catalogo, podeGer) {
    ...
    el.innerHTML = toast + btnAdd + seletor + conteudo;
  ...
  function carregarFeiticos() {
    ...
    Promise.all([
      ...
      renderFeiticos(uid, res[0] || {}, res[1] || {}, podeGer);
    }).catch(function() {
