  ...
  if(derrubadosAtual[time + '_' + pos]){
    ...
    var savesCaido = [
      fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/resultado/log/' + logIndexCaido, {
        text: slotCaido.nome + ' estava caído(a) e não conseguiu agir nesta fase.'
      }),
...
if(!logAbasEstado[idTabs]) logAbasEstado[idTabs] = { abaAtual: numeros[numeros.length - 1], contagens: {}, maiorFaseConhecida: numeros[numeros.length - 1] };
  ...
  function renderConteudoAba(){
    ...
    var scrollAntes = elConteudo.scrollTop;
    var estavaNoTopo = scrollAntes < 4;

    estado.contagens[n] = logFase.length;
    elConteudo.dataset.faseAtual = String(n);

elConteudo.innerHTML = logFase.slice().reverse().map(function(entrada){
    ...
    elConteudo.scrollTop = estavaNoTopo ? 0 : scrollAntes;
  }
