...
function detectarVassoura(item){
  ...
  // bônus fixo de combate: +2 se algum material for "Raro", senão +1 (padrão "Mágico")
  var ehRaro = /raro/i.test(desc);
  var bonus = ehRaro ? 2 : 1;

  // atributos percentuais da vassoura — não valem mais no combate direto,
  // só servem pra reduzir penalidade de eventos aleatórios de fase
...
function bonusVassoura(slot, tipoAcao){
  // o "tipo" deixou de importar: cada vassoura agora tem um bônus fixo único
  // (a raridade define o valor; os atributos percentuais só valem pra eventos)
...
// só as ações que têm rolagem de dado de verdade entram no treino
// (as puramente de suporte, tipo Passar/Proteger/Defender, não têm conta pra mostrar)
...
function montarSlotTreino(uid){
  return Promise.all([
    ...
    return {
      atributos: atributos,
      vassoura: { nome:'Vassoura Padrao de Hogwarts', bonus:1 }, // treino sempre usa a padrão, pra simplificar
      maestria: {
        quadribol: pctQuadribol,
        voo: 0,
        artilheiro: (cadastro.especializacoes && cadastro.especializacoes.artilheiro) || 0,
...
// versão "com detalhe" da fórmula, só pra treino — expõe cada termo da conta
// (a rolagem real de partida usa rolar(), que fica intacta e não é tocada aqui)
  ...
  fbGet(QUAD_FB_PARTIDAS, '/treinos/sessoes/' + sid + '/rodadas/' + rodadaAtual).then(function(rodada){
    ...
    if(rodadaTreinoRenderizada !== rodadaAtual){
      ...
      elAcoes.innerHTML = lista.map(function(a){
        return '<button type="button" class="q2rodadaacaobtn" data-acao="' + a.id + '">' + escaparHtml(a.nome) + '</button>';
      }).join('');
...
function renderResultadoRodada(sessao, rodada, souA){
  ...
  function contaHtml(r){
    return '<div class="q2rodadaconta">dado ' + r.dado + ' + atributos (' + r.principal + '+' + r.secundario + ')' +
      ' + vassoura ' + r.vassoura + ' + maestria (' + r.maestriaBase + '+' + r.maestriaEsp + ')' +
      ' = total <b>' + r.total + '</b>' + (r.critico20 ? ' — CRÍTICO NATURAL!' : '') + '</div>';
  }
