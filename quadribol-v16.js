...
if(itensLJ.length){
      ...
      lista.forEach(function(a){
        ...
        var bloqueado = ehCaca && (fase < QUAD_APANHADOR_CACA_FASE_MIN || cacaUsados >= QUAD_APANHADOR_CACA_MAX);
        var titleBloqueio = bloqueado
          ? (fase < QUAD_APANHADOR_CACA_FASE_MIN
...
function renderAlvo(sk, alvos, acaoId){
  ...
  fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(match){
    ...
    if(alvos === 'adversarios'){
      ...
      ['artilheiro1','artilheiro2'].forEach(function(p){ var s = match.times[t].slots[p]; if(s && podeMirar(t, p)) lista.push({ val:p, label:escaparHtml(s.nome) + ' (' + POS_LABEL[p] + ')' }); });
