(function () {
  ...
  var BG_CAMADAS = {
    superficie: 'url("https://cdna.artstation.com/p/assets/images/images/084/475/614/4k/junliang-zhang-1.jpg?1738470123")',
    subsolo:    'url("https://cdnb.artstation.com/p/assets/images/images/084/475/619/4k/junliang-zhang-4.jpg?1738470152")',
    profundeza: 'url("https://cdna.artstation.com/p/assets/images/images/084/475/624/4k/junliang-zhang-6.jpg?1738470171")'
  };
  ...
  /* ─── COOLDOWNS ──────────────────────────── */
  function cdPath(tipo, remUid, alvoUid) {
    return '/athenaeum/cd-'+tipo+'/'+diaAtual()+'/'+fkey(remUid)+'/'+fkey(alvoUid);
  }
  ...
  function executarDuelo(user, alvoUid, alvoNome, camadaId) {
    ...
    return verificarCd('duelo',user.uid,alvoUid).then(function(jaUsou){
      ...
      return Promise.all([getAtributos(user.uid),getAtributos(alvoUid)]).then(function(res){
        var atacVenceu=resolverCombate(res[0],res[1],0);
        ...
        return Promise.all([
          ...
          return dbPost('/athenaeum/duelos',reg).then(function(r){ return r.json(); }).then(function(d){
            ...
            ]).then(function(){ return { atacVenceu:atacVenceu, danoAtacante:danoAtacante, danoDefensor:danoDefensor }; });
          });
        });
      });
    });
        ...
        return Promise.all([
          ...
          return Promise.all([
            ...
            dbPost('/athenaeum/notificacoes/'+fkey(duelo.atacante_uid),{
              ...
              atacante_venceu:atacVenceu, dano_sofrido:danoAtacante,
              lido:false, ts:Date.now()
            })
          ]).then(function(){ return { atacVenceu:atacVenceu, danoDefensor:danoDefensor, danoAtacante:danoAtacante }; });
        });
      });
    });
  }
  ...
  function mostrarResultadoDuelo(venceu, danoMeu, danoOponente, nomeOponente) {
    abrirModal('Resultado do Duelo', function(mel){
      mel.innerHTML=
        '<div class="ath-duelo-res '+(venceu?'ath-res-vit':'ath-res-der')+'">'+
          ...
          (!venceu?'<div class="ath-duelo-aviso">'+nomeOponente+' tem 24h para contra-atacar.</div>':'')+
        '</div>';
    });
  }
      ...
      el.querySelectorAll('.ath-btn-duelo').forEach(function(btn){
        btn.addEventListener('click',function(){
          ...
          executarDuelo(user,btn.dataset.alvo,btn.dataset.nome,_camadaVis).then(function(res){
            ...
            mostrarResultadoDuelo(res.atacVenceu,res.danoAtacante,res.danoDefensor,btn.dataset.nome);
          });
        });
      });
  ...
  function renderPainelEntrada(user, camadaId, atributos) {
    ...
    function step1(){
      getLivros(user.uid).then(function(livros){
        painel.innerHTML=
          ...
          '<p class="ath-sb-hint">+'+cam.gph+'G/h \u00b7 '+cam.eph+'E/h</p>'+
    ...
    function step2(livro){
      Promise.all([getEnergia(user.uid),getHorasHoje(user.uid)]).then(function(res){
        ...
        if (restante<=0){ painel.innerHTML='<div class="ath-sb-titulo">Limite di\u00e1rio atingido</div><p class="ath-sb-hint">J\u00e1 estudou '+fmtHoras(horasHoje)+' hoje.</p>'; return; }
        painel.innerHTML=
  ...
  function renderPainelSessao(user, sessao, atributos) {
    var painel=document.getElementById('ath-painel-usuario'); if (!painel) return;
    var concluida=Date.now()>=sessao.termina_em, cam=getCamada(sessao.camada);
    painel.innerHTML=
  ...
  function renderNavCamadas(user, camadaSessao, atributos) {
    ...
    nav.innerHTML=CAMADAS.map(function(c){
      var pode=atributos&&(atributos.inteligencia||0)>=c.req_int, ativo=c.id===_camadaVis;
    ...
    nav.querySelectorAll('.ath-nav-btn:not([disabled]):not(.ath-nav-ativo)').forEach(function(btn){
      btn.addEventListener('click',function(){
        ...
        } else { mudarView(user,novaCam,atributos,camadaSessao); }
      });
    });
  }
  ...
  function renderSidebarInfo(user) {
    ...
    Promise.all([getNome(user.uid),getAtributos(user.uid)]).then(function(res){
      var nome=res[0], atr=res[1], int=atr.inteligencia||0;
      el.innerHTML=
  ...
  function renderTabs(user) {
    ...
    var abas=[{id:'historico',label:'Hist\u00f3rico'},{id:'duelos',label:'Duelos'}];
    ...
    sec.innerHTML=
      ...
      '<div class="ath-botoes-extra">'+
        ...
        '<span id="ath-badge-notif" style="display:none;position:absolute;top:-4px;right:-4px;background:#8b2222;color:#fff;border-radius:50%;width:16px;height:16px;font-size:10px;line-height:16px;text-align:center;"></span></button>'+
      '</div>';
      ...
      if (nids.length){
        ...
        nids.sort(function(a,b){ return notifs[b].ts-notifs[a].ts; }).forEach(function(id){
          ...
          html+='<div class="ath-notif-item'+(n.lido?'':' ath-notif-novo')+'">'+
            '<span class="ath-notif-txt">'+txt+'</span>'+danoInfo+
            '<div class="ath-notif-acoes">'+
            (podeCa?'<button class="ath-btn-mini ath-btn-ca" data-id="'+n.duelo_id+'" data-nid="'+id+'">Contra-atacar</button>':'')+
            '<button class="ath-btn-mini ath-btn-del-notif" data-id="'+id+'" style="color:#8b2222">\u00d7</button>'+
            '</div></div>';
        });
      }
      ...
      if (bids.length){
        ...
        bids.sort(function(a,b){ return bilhetes[b].ts-bilhetes[a].ts; }).forEach(function(id){
          ...
          html+='<div class="ath-bilhete-item'+(b.lido?'':' ath-bilhete-novo')+'">'+
            ...
            '<button class="ath-btn-mini ath-btn-del-bil" data-id="'+id+'" style="color:#8b2222;float:right">\u00d7</button>'+
            '</div>';
        });
      }
      ...
      el.querySelectorAll('.ath-btn-ca').forEach(function(btn){
        btn.addEventListener('click',function(){
          ...
          executarContraAtaque(user,btn.dataset.id,btn.dataset.nid).then(function(res){
            ...
            mostrarResultadoDuelo(!res.atacVenceu,res.danoDefensor,res.danoAtacante,'oponente');
  ...
  function renderRankDuelos(el) {
    ...
    dbGet('/athenaeum/ranking-duelos/'+mesAtual()).then(function(rank){
      ...
      el.innerHTML='<div class="ath-ranking"><p class="ath-ranking-mes">Top 10 Duelistas \u2014 '+m[1]+'/'+m[0]+'</p>'+
      ...
      getAtributos(user.uid).then(function(atr){
        dbGet('/athenaeum/ativas/'+fkey(user.uid)).then(function(sessao){
          ...
          if (sessao&&sessao.termina_em>Date.now()){
            camadaSessao=sessao.camada; _camadaVis=sessao.camada;
            atualizarBg(sessao.camada);
            renderPainelSessao(user,sessao,atr);
            iniciarTimer(sessao.inicio_em); iniciarStatusTick(sessao,user.uid);
            var header=document.getElementById('ath-main-header');
            if (header) header.textContent='Athenaeum \u2014 '+getCamada(sessao.camada).label;
          } else { renderPainelEntrada(user,_camadaVis,atr); }
