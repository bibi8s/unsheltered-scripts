...
async function carregarMochila() {
  ...
  console.log('inv-mochila encontrado:', el); // <-- adiciona isso
  var elSlots = document.getElementById('mochila-modal-slots');
  if (!uid || !el) return; 
  ...
  if (elSlots) {
    ...
    elSlots.innerHTML =
      '<span class="mochila-slots-label" style="color:' + cor + ';">' + usados + ' / ' + limite + ' espaços</span>' +
      (admin ?
      ...
      : '') +
      (travada ? '<span class="mochila-lock-label"><i class="ph ph-lock"></i> mochila travada</span>' : '');
  }
  ...
  var html = Object.entries(data).map(function(entry) {
    ...
    if (pode && !travada) {
      if (emCooldown) {
        btnBau = '<span onclick="gringToast(\'Este item só pode ser devolvido ao baú a partir da próxima segunda-feira.\', true)" class="btn-inv-blocked">bloqueado</span>';
      } else {
        btnBau = '<button onclick="moverParaBau(\'' + chave + '\')" class="btn-inv btn-inv-move" title="Devolver ao baú">&#8594; baú</button>';
      }
    }
    ...
    return '<div class="inv-item">' +
      ...
      (icone ? '<img src="' + icone + '" style="width:32px;height:32px;object-fit:contain;flex-shrink:0;">' : '') +
      '<span style="flex:1"><b>' + item.nome + '</b>' + qtd + '</span>' +
      (descricao ? '<span onclick="var i=this.querySelector(\'i\');i.classList.toggle(\'ph-arrow-circle-down\');i.classList.toggle(\'ph-arrow-circle-up\');document.getElementById(\'' + uid_item + '\').classList.toggle(\'aberto\')" class="inv-desc-toggle"><i class="ph-fill ph-arrow-circle-down"></i></span>' : '') +
      (pode && !travada ?
        '<div class="inv-item-btns">' +
        (admin ? '<button onclick="removerUmDaMochila(\'' + chave + '\')" class="btn-inv btn-inv-red"><i class="ph ph-minus-circle"></i></button>' : '') +
        btnBau +
        '</div>'
      : '') +
      '</div>' +
...
setTimeout(function () {
  ...
  overlay.id = 'mochila-modal-overlay';
  overlay.innerHTML =
    '<div id="mochila-modal">' +
      '<div class="mochila-modal-header">' +
        '<div class="mochila-modal-titulo"></div>' +
        '<button class="btn-fechar-mochila" id="btn-fechar-mochila"><i class="ph ph-x"></i></button>' +
      '</div>' +
      '<div class="mochila-modal-slots" id="mochila-modal-slots"></div>' +
      '<div class="mochila-modal-body"><div id="inv-mochila"></div></div>' +
    '</div>';
  document.body.appendChild(overlay);

  document.getElementById('btn-fechar-mochila').onclick = function() {
  ...
  setTimeout(function() {
    var slotBau = document.getElementById('inv-slots');
    if (slotBau && !slotBau.querySelector('.btn-abrir-mochila')) {
      var btnAbrir = document.createElement('button');
      btnAbrir.className = 'btn-abrir-mochila';
      btnAbrir.innerHTML = '<i class="ph ph-backpack"></i> Mochila';
      btnAbrir.onclick = function() { overlay.classList.add('aberto'); };
      slotBau.appendChild(btnAbrir);
    }
  }, 800);
