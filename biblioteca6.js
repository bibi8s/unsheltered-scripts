...
function abrirEntrada(user) {
  ...
  var abre  = (window.B_CONFIG && window.B_CONFIG.hora_abertura) || 6;
  var fecha = (window.B_CONFIG && window.B_CONFIG.hora_fechamento) || 23;

  if (hora < abre || hora >= fecha) {
    abrirModal('<p id="bibmsg">A biblioteca está fechada. Horário de funcionamento: ' + abre + 'h às ' + fecha + 'h.</p>');
    return;
  }

  dbGet('/biblioteca/historico/' + fkey(user.uid)).then(function(hist){
    hist = hist || {};
    var hojeHoras = Object.values(hist).filter(Boolean).reduce(function(acc, s){
      if (fmtDataISO(s.inicio_em) === diaAtual()) return acc + (s.horas_reais || 0);
      return acc;
    }, 0);

    if (hojeHoras >= 6) {
      abrirModal('<p id="bibmsg">Você já atingiu o limite de 6h de estudo hoje.</p>');
      return;
    }
...
function irParaAndar(num, user) {
  dbGet('/biblioteca/ativas/' + fkey(user.uid)).then(function(sessaoAtual){
    if (!sessaoAtual || sessaoAtual.termina_em <= Date.now()) {
      abrirModal('<p id="bibmsg">Você precisa estar estudando na biblioteca para acessar esta área.</p>');
      return;
    }
...
var imune  = a.id === 'atrapalhar' && s.mesa_id === 'fundo';
var usado;
if (imune) {
  usado = true;
} else if (a.id === 'atrapalhar' && noBanheiro) {
...
if (acao.bonus) {
    promessas.push(
      dbGet('/biblioteca/ativas/' + fkey(alvoUid)).then(function(sessao){
        if (!sessao) return;
        if (tipo === 'atrapalhar' && sessao.mesa_id === 'fundo') return;
...
function abrirBilhetes(user) {
  dbGet('/biblioteca/ativas/' + fkey(user.uid)).then(function(sessaoAtual){
    if (!sessaoAtual || sessaoAtual.termina_em <= Date.now()) {
      abrirModal('<p id="bibmsg">Você precisa estar estudando na biblioteca para acessar os bilhetes.</p>');
      return;
    }
...
function abrirLivros(user) {
  Promise.all([
    ...
    Object.values(hist).filter(Boolean).forEach(function(s) {
      ...
      if (dia.slice(0, 7) !== mesAtual()) return;
    ...
    document.querySelectorAll('.livromaestresg').forEach(function(btn) {
      btn.addEventListener('click', function() {
        ...
        dbPost('/inventario/' + fkey(user.uid), {
          ...
          return dbGet('/biblioteca/progresso/' + fkey(user.uid) + '/' + mesAtual() + '/maestrias');
        }).then(function(progAtual) {
          progAtual = progAtual || { resgatadas: [], livros: {} };
          var novasResgatadas = (progAtual.resgatadas || []).concat([slug]);
          return Promise.all([
            dbPut('/biblioteca/progresso/' + fkey(user.uid) + '/' + mesAtual() + '/maestrias/resgatadas', novasResgatadas),
            dbPut('/biblioteca/progresso/' + fkey(user.uid) + '/' + mesAtual() + '/maestrias/livros/' + encodeURIComponent(livro), { resgatado: true, maestria: slug })
          ]).then(function(){
          ...
          if (resgatadas.length >= 2) {
            document.querySelectorAll('.livromaestresg').forEach(function(b){ b.disabled = true; });
          }
        });
      });
    });
...
/* ===================== ENCERRAR SESSAO ===================== */
function fecharSessaoPorExpiracao(sessao) {
  var agora      = Date.now();
  ...
  return Promise.all([
    fetch(DB + '/biblioteca/ativas/' + fkey(sessao.uid) + '.json', { method: 'DELETE' }),
    dbPost('/biblioteca/historico/' + fkey(sessao.uid), {
    ...
    atualizarProgresso(sessao.uid, horasEfet),
    atualizarRanking(sessao.uid, sessao.nome, horasEfet)
  ]);
}
...
function encerrarSessao(user) {
  ...
  dbGet('/biblioteca/ativas/' + fkey(user.uid)).then(function(sessao) {
    ...
    fecharSessaoPorExpiracao(sessao).then(function(){
      trocarAndar(1);
      mostrarEntrada();
...
function carregarBiblioteca(user) {
  dbGet('/biblioteca/ativas').then(function(ativas){
    ativas = ativas || {};
    var agora     = Date.now();
    var expiradas = Object.keys(ativas).filter(function(k){
      return ativas[k] && ativas[k].termina_em <= agora;
    });

    Promise.all(expiradas.map(function(k){ return fecharSessaoPorExpiracao(ativas[k]); }))
      .then(function(){
        if (!expiradas.length) return ativas;
        return dbGet('/biblioteca/ativas').then(function(novas){ return novas || {}; });
      })
      .then(function(ativasAtualizadas){
        renderCards(ativasAtualizadas, user);
      });
  });
  atualizarBadges(user);
}
