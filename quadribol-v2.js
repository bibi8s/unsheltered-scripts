...
/* ============================================================
   BLOCO -1 — Constantes globais do jogo
   (precisam estar aqui no JS, não no HTML, pois <script> embutido
   no bloco de HTML desta plataforma não é executado)
   ============================================================ */
...
var QUAD_ADMINS = ['1']; // uids (sem o "u") que são admin/staff
...
/* --- pomo de ouro (mecânica v2) --- */
var QUAD_POMO_MIN = 10;
var QUAD_POMO_MAX = 14;
var QUAD_POMO_BONUS_FASES = 5;

/* --- postura de time (escolhida pelo capitão no lobby, travada a partida toda) --- */
var QUAD_POSTURAS = {
...
/* --- combos entre posições (referência — a lógica real vive no Bloco 11) --- */
var QUAD_COMBOS = [
...
/* --- boost (1x por partida, dobra vassoura+maestria numa rolagem) --- */
...
/* ============================================================
   BLOCO 0 — Troca das 4 abas principais (Campo/Treinos/Partidas/Painel)
   ============================================================ */
...
/* ============================================================
   BLOCO 1 — Conexão Firebase + usuário
   ============================================================ */
...
/* ============================================================
   BLOCO 2 — boot() decide o que mostrar na aba Campo
   ============================================================ */
...
function renderLobby(match){
  ...
  document.getElementById('q3nomea').textContent = timeA.nome;
  document.getElementById('q3nomeb').textContent = timeB.nome;
  document.getElementById('q3colnomea').textContent = timeA.nome;
  document.getElementById('q3colnomeb').textContent = timeB.nome;
  ...
  montarColuna('q3cola', timeA.slots);
  montarColuna('q3colb', timeB.slots);
}

function montarColuna(id, slots){
...
function detectarVassoura(item){
  ...
  if(desc.indexOf('Cabo:') === -1 && !/Percorre\s+\d+\s+metros/i.test(desc)) return null;
  var bonus = 1;
  var mano = desc.match(/Manobrabilidade:\s*(\d+)%/i);
  if(mano){
    bonus = Math.max(1, Math.floor(parseInt(mano[1]) / 10));
  }else{
    var perc = desc.match(/Percorre\s+(\d+)\s+metros/i);
    if(perc) bonus = Math.max(1, Math.floor(parseInt(perc[1]) / 10));
  }
  return { nome: item.nome || 'Vassoura', bonus: bonus };
}
...
function confirmarPresenca(meu){
  ...
  Promise.all([
    ...
    var pctFinal = pctQuadribol > 0 ? pctQuadribol : pctVoo;
...
/* ============================================================
   BLOCO 5 — Renderização do Jogo em andamento
   ============================================================ */
...
var elJog = document.getElementById('q3jogadores');
  ...
  document.getElementById('q3placarnomea').textContent = timeA.nome;
  document.getElementById('q3placarnomeb').textContent = timeB.nome;
...
/* ============================================================
   BLOCO 6 — Escolher e confirmar ação
   ============================================================ */
...
/* ============================================================
   BLOCO 7 — Avançar turno (fila individual)
   ============================================================ */
...
/* ============================================================
   BLOCO 8a — Fórmula de rolagem (1d20 + modificadores + crítico)
   ============================================================ */
...
function bonusVassoura(slot, tipoAcao){
  ...
  var v = slot.vassoura;
  if(tipoAcao === 'velocidade') return v.velocidade || v.bonus || 0;
  if(tipoAcao === 'manobrabilidade') return v.manobrabilidade || v.bonus || 0;
  if(tipoAcao === 'estabilidade') return v.estabilidade || v.bonus || 0;
  return v.bonus || 0;
}
...
function rolar(slot, atributoPrincipal, atributoSecundario, opcoes){
  ...
  var evento = bonusEvento(opcoes.evento, opcoes.pos);
...
/* ============================================================
   BLOCO 8c — Resolver Apanhador + Pomo de Ouro (mecânica v2)
   ============================================================ */
...
/* ============================================================
   BLOCO 8d — Fechar a fase (placar, debuffs, Pomo, fim de jogo)
   ============================================================ */
...
/* ============================================================
   BLOCO 8b — Resolver ações de Artilheiros, Goleiro e Batedor
   ============================================================ */
...
/* ============================================================
   BLOCO 9 — Escolha de ordem pelo capitão (início de cada fase)
   ============================================================ */
...
/* ============================================================
   BLOCO 10 — Timer de turno + delegação pro capitão + NPC
   ============================================================ */
...
/* ============================================================
   BLOCO 11 — Detectar combos entre posições (mesma fase)
   ============================================================ */
...
/* ============================================================
   BLOCO 12 — Evento aleatório de fase
   ============================================================ */
...
/* ============================================================
   BLOCO 13 — Ação de liderança do capitão (Motivar/Estratégia)
   ============================================================ */
...
/* ============================================================
   BLOCO 14 — Tela de fim de partida
   ============================================================ */
...
function renderFim(match){
  ...
  document.getElementById('q3fimnomea').textContent = timeA.nome;
  document.getElementById('q3fimnomeb').textContent = timeB.nome;
  ...
  if(v === 'A'){
    elResultado.textContent = timeA.nome + ' venceu!';
  }else if(v === 'B'){
    elResultado.textContent = timeB.nome + ' venceu!';
  }else{
...
/* ============================================================
   BLOCO 15 — Partidas: histórico e ranking
   ============================================================ */
    ...
    entradas.forEach(function(h){
      ...
      div.innerHTML =
        '<div class="q4itemtopo">' +
          '<div class="q4itemplacar">' + escaparHtml(h.nomeA) + ' ' + h.placarA + ' × ' + h.placarB + ' ' + escaparHtml(h.nomeB) + '</div>' +
          '<div class="q4itemdata">' + dataStr + '</div>' +
        '</div>' +
        '<div class="q4itemvencedor">' + (h.nomeVencedor ? h.nomeVencedor + ' venceu' : 'Empate') +
...
function renderRanking(){
  ...
  fbGet(QUAD_FB_PARTIDAS, '/historico').then(function(historico){
    ...
    function garantir(nome){
      if(!stats[nome]) stats[nome] = { vitorias:0, derrotas:0, empates:0 };
    }
    ...
    var nomes = Object.keys(stats).sort(function(a, b){ return stats[b].vitorias - stats[a].vitorias; });

    lista.innerHTML = '';
    nomes.forEach(function(nome){
      var s = stats[nome];
      var row = document.createElement('div');
      row.className = 'q4rankrow';
      row.innerHTML =
        '<div class="q4ranknome">' + escaparHtml(nome) + '</div>' +
        '<div class="q4rankvd"><b>' + s.vitorias + 'V</b> ' + s.derrotas + 'D ' + s.empates + 'E</div>';
...
/* ============================================================
   BLOCO 16 — Torcida, apostas e vuvuzela (espectador)
   ============================================================ */
...
/* ----- vuvuzela: poll rápido (1s), separado do poll geral (2.5s) ----- */
...
function renderTreinoForm(){
  document.getElementById('q2form').hidden = false;
  document.getElementById('q2prev').textContent = '';
  document.getElementById('q2uid').value = '';

  document.getElementById('q2uid').onblur = function(){
...
function processarFimTreino(sid, sessao, semana){
  fbGet(QUAD_FB_PARTIDAS, '/treinos/jogadores/' + myUid).then(function(dados){
    dados = dados || {};
    var horasAntes = (dados.semana === semana) ? (dados.horas_semana || 0) : 0;
    var horasDepois = horasAntes + sessao.duracao_h;

    Promise.all([
      fbPatch(QUAD_FB_PARTIDAS, '/treinos/jogadores/' + myUid, {
        semana: semana, horas_semana: horasDepois, sessao_id: null
      }),
      fbPut(QUAD_FB_PARTIDAS, '/treinos/sessoes/' + sid + '/status', 'concluida')
    ]).then(function(){
      esconderEstadosTreino();
      document.getElementById('q2concluido').hidden = false;
      document.getElementById('q2concluidotxt').textContent =
        '+' + sessao.duracao_h + 'h registradas. ' + horasDepois.toFixed(1) + 'h no total esta semana.';
...
/* ============================================================
   BLOCO 18 — Painel: cadastro de Jogadores (real ou NPC)
   ============================================================ */
...
function carregarFormularioNpc(id, cad){
  ...
  document.getElementById('q5jognpc').onchange(); // reaproveita a lógica que alterna os campos
  ...
  fbGet(QUAD_FB_PARTIDAS, '/jogadores').then(function(jogadores){
    ...
    Object.keys(jogadores).forEach(function(id){
      ...
      row.innerHTML =
        ...
        '<div class="q5itemmeta">' + escaparHtml(j.casa) + ' · ' + escaparHtml(j.posicao) + (j.titular ? ' · titular' : ' · reserva') + '</div>' +
...
/* ============================================================
   BLOCO 18b — Painel: cadastro de Comentaristas
   (registro global — define quem pode postar como comentarista
   oficial no mural de qualquer partida)
   ============================================================ */
...
/* ============================================================
   BLOCO 19 — Jogadores por casa (titulares e reservas)
   visível para todos, dentro da aba Partidas → "times por casa"
   ============================================================ */
...
function iniciarJogadoresPorCasa(){
  ...
  nav.innerHTML = QUAD_CASAS.map(function(c, i){
    return '<button type="button" class="q5casatab' + (i === 0 ? ' on' : '') + '" data-casa="' + c + '">' + c + '</button>';
  }).join('');
...
function renderJogadoresDaCasa(casa){
  ...
  fbGet(QUAD_FB_PARTIDAS, '/jogadores').then(function(jogadores){
    ...
    todos.forEach(function(j){
      var fotoHtml = j.foto
        ? '<img class="q5cardfoto" src="' + j.foto + '">'
...
/* ============================================================
   BLOCO 21 — Painel: Treinos (visão admin — controle geral)
   ============================================================ */
...
/* ============================================================
   BLOCO 22 — Painel: navegação entre sub-abas + inicialização
   ============================================================ */
...
document.getElementById('q1tadm').addEventListener('click', function(){
  iniciarPainelCriar(); // aba padrão do painel é "criar partida"
});
/* ============================================================
   BLOCO 23 — Ficha do jogador (modal)
   ============================================================ */
...
function abrirFicha(match, time, pos){
  ...
  document.getElementById('q3fichapos').textContent = POS_LABEL[pos] + ' — ' + match.times[time].nome;
