(function () {
  ...
  var GRUPOS_BASE = [
    { id: 'corporais',     label: 'Corporais'    },
    { id: 'eruditas',      label: 'Eruditas'     },
  ...
  var MAXIMOS = {
    'bruxo':        { corporais: 700, eruditas: 400, influencia: 500, sobrevivencia: 350, oficios: 500 },
    'meio-criatura':{ corporais: 800, eruditas: 350, influencia: 400, sobrevivencia: 400, oficios: 400 },
    'criatura':     { corporais: 900, eruditas: 300, influencia: 450, sobrevivencia: 500, oficios: 450 }
  };
  ...
  function getTipoPersonagem() {
    ...
    var especieTexto  = especie  ? especie.textContent.trim().toLowerCase()  : '';
    ...
    if (especieTexto.indexOf('criatura') !== -1) return 'criatura';
    return 'bruxo';
  }

  function getGrupos() {
    var tipo = getTipoPersonagem();
    var maxs = MAXIMOS[tipo] || MAXIMOS['bruxo'];
    return GRUPOS_BASE.map(function(g) {
      return { id: g.id, label: g.label, max: maxs[g.id] };
    });
  }
    ...
    GRUPOS.forEach(function(g,i){
      ...
      paineis +=
        '<div class="maest-painel'+(i===0?'':' maest-oculto')+'" data-grupo="'+g.id+'">'+
          ...
          '<div class="maest-cards-grid">'+conteudo+'</div>'+
        '</div>';
    });
