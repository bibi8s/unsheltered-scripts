(function () {
  ...
  function renderHabilidades(uid, habCat, maldCat, perso, nivel, admin) {
    ...
    var html = toast;
    if (temHab || admin)  html += montarSecao('Habilidade',  cardsHab  || '<div class="apti-vazio">Nenhuma.</div>', admin ? 'btn-add-hab'  : null, admin ? 'sel-hab'  : null);
    if (temMald || admin) html += montarSecao('Maldi\u00e7\u00e3o', cardsMald || '<div class="apti-vazio">Nenhuma.</div>', admin ? 'btn-add-mald' : null, admin ? 'sel-mald' : null);

    el.innerHTML = html;

    if (admin) {
      if (temHab  || admin) bindSeletor('btn-add-hab',  'sel-hab',  uid, 'habilidade');
      if (temMald || admin) bindSeletor('btn-add-mald', 'sel-mald', uid, 'maldicao');
    }
  }
  ...
  function carregarHabilidades() {
    ...
    Promise.all([
      ...
      renderHabilidades(uid, res[0], res[1], res[2], nivel, isAdmin());
    }).catch(function(){
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(function(){ tentarCarregar(20); }, 2000); });
} else {
  setTimeout(function(){ tentarCarregar(20); }, 2000);
}
