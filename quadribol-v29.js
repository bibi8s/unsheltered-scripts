...
function mostrarVazio(){
  ...
  document.getElementById('q3fim').hidden = true;
...
function renderLobby(match){
  ...
  var elRowDataHora = document.getElementById('q3rowdatahora');
  if(match.data_agendada || match.hora_agendada){
    elRowDataHora.hidden = false;
    var dataFormatada = '';
    if(match.data_agendada){
      var partesData = match.data_agendada.split('-');
      dataFormatada = partesData[2] + '/' + partesData[1] + '/' + partesData[0];
    }
    document.getElementById('q3datahora').textContent =
      (dataFormatada || 'data não definida') + (match.hora_agendada ? ' às ' + match.hora_agendada : '');
  }else{
    elRowDataHora.hidden = true;
  }
...
document.getElementById('q3trocaconfirmar').onclick = function(){
  ...
  Promise.all([
    ...
    if(cad.isNpc){
      ...
      novoSlot.vassoura = cad.vassoura || { nome:'Vassoura Padrao de Hogwarts', bonus:0 };
      novoSlot.maestria = {
...
function forcarAvancarTurnoAdmin(){
  if(!isAdmin || !pid) return;
  ...
  fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(match){
    ...
    if(!atual){ alert('Não achei um turno atual pra avançar.'); return; }
...
document.getElementById('q3admavancar').onclick = forcarAvancarTurnoAdmin;
document.getElementById('q3admresetfase').onclick = resetarFaseAdmin;

var QUAD_VASSOURAS_BONUS_ZERO = ['ember dash', 'hogwarts house'];

function normalizarNomeVassoura(nome){
...
function detectarVassoura(item){
  ...
  var nomeNormalizado = normalizarNomeVassoura(item.nome);
  var ehBonusZero = QUAD_VASSOURAS_BONUS_ZERO.indexOf(nomeNormalizado) !== -1;
  var ehRaro = /raro/i.test(desc);
  var bonus = ehBonusZero ? 0 : (ehRaro ? 2 : 1);
...
function carregarVassouras(){
  ...
  Promise.all([
    ...
    var itensArr = Object.values(todos);

    var buscasCatalogo = itensArr
      .filter(function(item){ return item && !item.descricao && item.item_id; })
      .map(function(item){
        return fbGet(QUAD_FB_PERSONAGENS, '/itens/' + item.item_id).then(function(catalogo){
          if(catalogo && catalogo.descricao) item.descricao = catalogo.descricao;
        });
      });

    return Promise.all(buscasCatalogo).then(function(){ return itensArr; });
  }).then(function(itensArr){
    var opts = '<option value="padrao">Vassoura Padrão de Hogwarts (+0)</option>';
    var lista = [];
    itensArr.forEach(function(item){
...
function confirmarPresenca(meu){
  ...
  var vassoura = (sel.value === 'padrao' || !sel.value)
    ? { nome:'Vassoura Padrao de Hogwarts', bonus:0 }
    : (lista[parseInt(sel.value)] || { nome:'Vassoura Padrao de Hogwarts', bonus:0 });
...
document.getElementById('q3muralbolha').onclick = function(){
    document.getElementById('q3muralpainel').hidden = false;
    document.getElementById('q3muralbolha').hidden = true;
  };
  document.getElementById('q3muralfechar').onclick = function(){
    document.getElementById('q3muralpainel').hidden = true;
    document.getElementById('q3muralbolha').hidden = false;
  };
...
var muralUltimaAssinatura = null;

function renderMural(match){
  var lista = document.getElementById('q3murallista');
  var fixo = document.getElementById('q3muralfixo');
  if(!lista) return;

  var mural = match.mural || {};
  var todasEntradas = Object.keys(mural).map(function(k){ return mural[k]; })
    .sort(function(a, b){ return (a.ts || 0) - (b.ts || 0); });

  var assinatura = todasEntradas.length + '_' + (todasEntradas.length ? todasEntradas[todasEntradas.length - 1].ts : 0);
  if(assinatura === muralUltimaAssinatura) return;
  muralUltimaAssinatura = assinatura;

  var fixas = todasEntradas.filter(function(m){ return m.tipo === 'comentarista'; });
  var normais = todasEntradas.filter(function(m){ return m.tipo !== 'comentarista'; }).slice(-50);

  if(fixo){
    if(fixas.length){
      var ultimaFixa = fixas[fixas.length - 1];
      fixo.innerHTML = '<i class="fa fa-bullhorn"></i><span class="q3muralnome">' + escaparHtml(ultimaFixa.nome) + ':</span> ' + escaparHtml(ultimaFixa.texto);
      fixo.hidden = false;
    }else{
      fixo.hidden = true;
    }
  ...
  if(!normais.length){
	...
	lista.innerHTML = '';
  normais.slice().reverse().forEach(function(m){
    var div = document.createElement('div');
    div.className = 'q3muralmsg';
...
function renderJogo(match){
  ...
  var elFaseTurno = document.getElementById('q3faseturno');
  if(elFaseTurno){
    var nomeTimeAtual = atual.time ? match.times[atual.time].nome : '—';
    var posAtual = atual.pos ? POS_LABEL[atual.pos] : '—';
    elFaseTurno.textContent = 'Fase ' + fase + ' de ' + QUAD_FASES_TOTAL + ' — Vez de: ' + nomeTimeAtual + ' (' + posAtual + ')';
  }

var elJog = document.getElementById('q3jogadores');
  elJog.innerHTML = '';
  ['A','B'].forEach(function(t){
    var coresCasaTime = corPrincipalCasa(match.times[t].nome);
    var ordemTime = faseObj['ordem_' + t] || POSICOES;

    var boxTime = document.createElement('div');
    boxTime.className = 'q3timebox';

    var header = document.createElement('div');
    header.className = 'q3timeheader';
    header.textContent = 'Time ' + t + ' — ' + (match.times[t].nome || '');
    boxTime.appendChild(header);

    var lista = document.createElement('div');
    lista.className = 'q3timelista';

    ordemTime.forEach(function(pos){
      var slot = match.times[t].slots[pos];
      if(!slot) return;
      ...
      div.dataset.time = t;
      div.dataset.pos = pos;
      if(coresCasaTime) div.style.setProperty('--casa-stroke', coresCasaTime);
      var badges = montarBadgesJogador(faseObj, t, pos, slot);
      var badgesHtml = badges.length ? '<div class="q3pbadges">' + badges.map(function(b){
        return '<span class="q3pbadge q3pbadge-' + b.tipo + '">' + escaparHtml(b.texto) + '</span>';
      }).join('') + '</div>' : '';
div.innerHTML = statusTurnoIcon(faseObj, t, pos, ehAtual) + '<div class="q3pnome">' + escaparHtml(abreviarNome(slot.nome)) + '</div><div class="q3ppos">' + POS_LABEL[pos] + '</div>' + badgesHtml;
		lista.appendChild(div);
    });

    boxTime.appendChild(lista);
    elJog.appendChild(boxTime);
  });

function montarBadgesJogador(faseObj, time, pos, slot){
  var sk = time + '_' + pos;
  var acoesFase = faseObj.acoes || {};
  var badges = [];

  if(slot.debuff){
    badges.push({ texto: '-2 ' + (ATTR_LABEL[slot.debuff.atributo] || slot.debuff.atributo), tipo:'debuff' });
  }
  if(slot.mergulhoBonus){
    badges.push({ texto: '+' + slot.mergulhoBonus + ' arremesso', tipo:'bonus' });
  }
  if(slot.exposto){
    badges.push({ texto: 'exposto', tipo:'debuff' });
  }
  if((faseObj.derrubados || {})[sk]){
    badges.push({ texto: 'caído', tipo:'debuff' });
  }
  if((faseObj.protegidos || {})[sk]){
    badges.push({ texto: 'protegido', tipo:'bonus' });
  }
  if((faseObj.fintouB || {})[sk]){
    badges.push({ texto: 'imune a balaço', tipo:'bonus' });
  }
  if((faseObj.bonusSituacional || {})[sk]){
    badges.push({ texto: '+' + faseObj.bonusSituacional[sk] + ' bônus', tipo:'bonus' });
  }
  if((faseObj.pendentes || {})[sk]){
    badges.push({ texto: 'aguardando goleiro', tipo:'pendente' });
  }

  return badges;
}

function statusTurnoIcon(faseObj, time, pos, ehAtual){
  var sk = time + '_' + pos;
  var ac = (faseObj.acoes || {})[sk];
  if(ac && ac.resolvida){
    return '<i class="fa fa-check q3pstatus q3pstatus-feito" title="já agiu"></i>';
  }
  if(ehAtual || ac){
    return '<i class="fa fa-hourglass-half q3pstatus q3pstatus-espera" title="esperando ação"></i>';
  }
  return '<i class="fa fa-clock q3pstatus q3pstatus-aguarda" title="ainda não agiu"></i>';
}
...
renderLogAbas('q3logabas', 'q3log', match.fases, mapaCoresNomesJogadores(match));  
  ...
  if(turnoIdx !== lastTurnoRenderizado){
    ...
    if(atual.time && atual.pos){
      var slotAtual = match.times[atual.time].slots[atual.pos];
      if(slotAtual && slotAtual.uid === myUid){
        tocarSomMinhaVez();
      }
    }
  }
...
function tocarSomMinhaVez(){
  ...
  try{
    [660, 880].forEach(function(freq, i){
      ...
      var inicio = ctx.currentTime + i * 0.12;
      ...
      osc.start(inicio); osc.stop(inicio + 0.25);
    });
  }catch(e){}
}
...
var ACOES = {
  artilheiro:[
    { id:'arremessar', nome:'Arremessar', desc:'Tenta marcar gol contra o Goleiro adversário.', icon:'fa-crosshairs' },
    { id:'passar', nome:'Passar', desc:'Passa a Goles para o parceiro. +2 no arremesso.', alvos:'art_aliados', icon:'fa-share' },
    { id:'fintar', nome:'Fintar', desc:'Esquiva do Balaço usando agilidade e leitura de jogo.', icon:'fa-random' },
    { id:'mergulho', nome:'Mergulho', desc:'Avança em alta velocidade. Vencendo: +4 arremesso.', icon:'fa-long-arrow-down' },
    { id:'finta_passe', nome:'Finta de Passe', desc:'Engana o Goleiro. Vencendo: -3 na defesa desta fase.', icon:'fa-magic' }
  ],
  batedor:[
    { id:'lancar_balaco', nome:'Lançar Balaço', desc:'Acerta um adversário. -2 no atributo principal.', alvos:'adversarios', icon:'fa-circle' },
    { id:'proteger', nome:'Proteger', desc:'Cobre um aliado contra o Balaço adversário.', alvos:'aliados', icon:'fa-shield' },
    { id:'interceptar', nome:'Interceptar', desc:'Bloqueia o passe dos Artilheiros adversários.', icon:'fa-ban' },
    { id:'derrubar', nome:'Derrubar', desc:'Colisão direta. Vencendo: alvo perde o próximo par.', alvos:'adversarios', icon:'fa-hand-rock-o' },
    { id:'pressao_aerea', nome:'Pressão Aérea', desc:'Intimida adversário. Vencendo: -2 no próximo roll.', alvos:'adversarios', icon:'fa-cloud' }
  ],
  goleiro:[
    { id:'defender', nome:'Defender', desc:'Posiciona-se nos aros. +2 no roll de defesa.', icon:'fa-shield' },
    { id:'blitz', nome:'Blitz', desc:'Avança para interceptar. Se falhar, gol automático.', icon:'fa-bolt' },
    { id:'lancar_goles', nome:'Lançar Goles', desc:'Arremessa a Goles para um Artilheiro aliado (+2).', alvos:'art_aliados', icon:'fa-share' },
    { id:'antecipar', nome:'Antecipar', desc:'Lê o jogo. Vencendo: +5 defesa esta fase.', icon:'fa-eye' },
    { id:'derrubar_artilheiro', nome:'Derrubar Artilheiro', desc:'Vencendo: artilheiro não arremessa.', alvos:'art_adversarios', icon:'fa-user-times' }
  ],
  apanhador:[
...
if(item.viaCapitao && !eMinhaVez) return;
    ...
    if(jaAgiu){
      ...
      var faseObjLJ = (match.fases && match.fases[fase]) || {};
      var logFaseLJ = (faseObjLJ.resultado && faseObjLJ.resultado.log) || [];
      var itensLJ = filtrarLogJogador(logFaseLJ, item.time, item.pos);
if(itensLJ.length){
        var mapaCoresLJ = mapaCoresNomesJogadores(match);
		html += '<div id="q3logjogador">' + itensLJ.map(function(it){
          return '<div class="q3logjogadorlinha' + (it.destaque ? ' q3logjogadordestaque' : '') + '">' + colorirNomesNoLog(escaparHtml(it.texto), mapaCoresLJ) + spoilerDadosHtml(it.dados) + '</div>';
        }).join('') + '</div>';
      }
      var cacaUsados = (item.slot.cacaUsados || 0);

      if(item.pos === 'apanhador'){
        html += '<div id="q3rastreiotxt">rastreio atual: ' + (item.slot.rastreamento || 0) +
          ' · turnos de caça usados: ' + cacaUsados + '/' + QUAD_APANHADOR_CACA_MAX + '</div>';
      }
      html += '<div class="q3acoesgrid">';

 var advTimeItem = item.time === 'A' ? 'B' : 'A';
      var apanhadorAdvJaAgiu = !!acoesFase[advTimeItem + '_apanhador'];

      lista.forEach(function(a){
        var ehCaca = item.pos === 'apanhador' && QUAD_APANHADOR_ACOES_CACA.indexOf(a.id) !== -1;
        var ehDistracaoInutil = a.id === 'distracao' && apanhadorAdvJaAgiu;
        var bloqueado = (ehCaca && (fase < QUAD_APANHADOR_CACA_FASE_MIN || cacaUsados >= QUAD_APANHADOR_CACA_MAX)) || ehDistracaoInutil;
        var titleBloqueio = '';
        if(ehDistracaoInutil){
          titleBloqueio = ' title="o Apanhador adversário já agiu nesta fase — não tem mais o que distrair"';
        }else if(ehCaca && bloqueado){
          titleBloqueio = fase < QUAD_APANHADOR_CACA_FASE_MIN
            ? ' title="só libera a partir da fase ' + QUAD_APANHADOR_CACA_FASE_MIN + '"'
            : ' title="já usou os ' + QUAD_APANHADOR_CACA_MAX + ' turnos de caça desta partida"';
        }
html += '<button type="button" class="q3acaobtn" data-sk="' + sk + '" data-acao="' + a.id + '" data-alvos="' + (a.alvos || '') + '"' +
          (bloqueado ? ' disabled' : '') + titleBloqueio + '><i class="fa ' + (a.icon || 'fa-circle') + '"></i> ' + a.nome + (bloqueado ? ' <i class="fa fa-lock"></i>' : '') + '</button>';
      });
      html += '</div>';
...
function gerarPainelDecisaoCapitao(time, pos, match){
  ...
  html += '<div class="q3acoesgrid">';
lista.forEach(function(a){
    html += '<button type="button" class="q3acaobtn" data-sk="' + sk + '" data-acao="' + a.id + '" data-alvos="' + (a.alvos || '') + '"><i class="fa ' + (a.icon || 'fa-circle') + '"></i> ' + a.nome + '</button>';
  });
  html += '</div>';
  html += '<div class="q3alvo" id="q3alvo_' + sk + '" hidden></div>';
...
if(!acaoSelecionada[sk]) acaoSelecionada[sk] = {};
      ...
      renderAlvo(sk, btn.dataset.alvos, btn.dataset.acao);
    };
  });
...
var ACOES_ALVO_REQUER_PENDENTE = ['auxiliar','inspirar','lancar_goles','passar','derrubar_artilheiro','derrubar','pressao_aerea'];

function renderAlvo(sk, alvos, acaoId){
  ...
  fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(match){
    ...
    var fase = match.fase_atual;
    var acoesFase = ((match.fases && match.fases[fase]) || {}).acoes || {};
    var exigirPendente = acaoId && ACOES_ALVO_REQUER_PENDENTE.indexOf(acaoId) !== -1;

    function podeMirar(timeAlvo, posAlvo){
      if(!exigirPendente) return true;
      var acAlvo = acoesFase[timeAlvo + '_' + posAlvo];
      return !(acAlvo && acAlvo.resolvida);
    }

    if(alvos === 'adversarios'){
      POSICOES.forEach(function(p){ var s = match.times[advT].slots[p]; if(s && podeMirar(advT, p)) lista.push({ val:p, label:escaparHtml(s.nome) + ' (' + POS_LABEL[p] + ')' }); });
    }else if(alvos === 'aliados'){
      POSICOES.forEach(function(p){ if(p === pos) return; var s = match.times[t].slots[p]; if(s && podeMirar(t, p)) lista.push({ val:p, label:escaparHtml(s.nome) + ' (' + POS_LABEL[p] + ')' }); });
	}else if(alvos === 'art_aliados'){
      ['artilheiro1','artilheiro2'].forEach(function(p){ if(p === pos) return; var s = match.times[t].slots[p]; if(s && podeMirar(t, p)) lista.push({ val:p, label:escaparHtml(s.nome) + ' (' + POS_LABEL[p] + ')' }); });
    }else if(alvos === 'art_adversarios'){
      ['artilheiro1','artilheiro2'].forEach(function(p){ var s = match.times[advT].slots[p]; if(s && podeMirar(advT, p)) lista.push({ val:p, label:escaparHtml(s.nome) + ' (' + POS_LABEL[p] + ')' }); });
    }

    if(!lista.length){
      wrap.innerHTML = '<div class="q3alvovazio" style="opacity:.5;font-size:11px;">nenhum alvo disponível — todos já jogaram nesta fase.</div>';
      wrap.hidden = false;
      return;
    }
...
function confirmarAcao(time, pos, acaoId, alvo, usouBoost){
  var sk = time + '_' + pos;
  if(acoesEmEnvio[sk]) return;
  acoesEmEnvio[sk] = true;

  fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(match){
    if(!match){ acoesEmEnvio[sk] = false; return; }
    var fase = match.fase_atual;
    ...
    Promise.all(promessas).then(function(){
      ...
      return resolverAcaoUnicaSequencial(pid, matchAtual, fase, time, pos);
    }).then(function(){
      acoesEmEnvio[sk] = false;
      avancarTurno();
    });
  });
}
...
function avancarTurno(){
  fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(match){
    ...
    if(proximoIdx >= fila.length){
      fecharFaseSequencial(pid, match, fase);
    }else{
      fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid, {
...
function bonusInspirado(acoes, time, pos){
  var acAp = acoes[time + '_apanhador'];
  if(acAp && acAp.acao === 'inspirar' && acAp.alvo === pos) return 2;
  return 0;
}


function somarMvpSeq(pid, sk, valor){
  var caminho = '/partidas/' + pid + '/pontuadores/' + sk;
  return fbGet(QUAD_FB_PARTIDAS, caminho).then(function(atual){
    return fbPut(QUAD_FB_PARTIDAS, caminho, (atual || 0) + valor);
  });
}

function logSequencialProxIndex(match, faseNum){
  var logObj = (match.fases && match.fases[faseNum] && match.fases[faseNum].resultado && match.fases[faseNum].resultado.log) || {};
  var chaves = Object.keys(logObj).map(Number);
  return chaves.length ? Math.max.apply(null, chaves) + 1 : 0;
}

function darBonusSituacionalSeq(pid, faseNum, timeAlvo, posAlvo, valor){
  var caminho = '/partidas/' + pid + '/fases/' + faseNum + '/bonusSituacional/' + timeAlvo + '_' + posAlvo;
  return fbGet(QUAD_FB_PARTIDAS, caminho).then(function(atual){
    return fbPut(QUAD_FB_PARTIDAS, caminho, (atual || 0) + valor);
  });
}

function consumirBonusSituacionalSeq(pid, faseNum, time, pos){
  var caminho = '/partidas/' + pid + '/fases/' + faseNum + '/bonusSituacional/' + time + '_' + pos;
  return fbGet(QUAD_FB_PARTIDAS, caminho).then(function(valor){
    if(!valor) return 0;
    return fbDel(QUAD_FB_PARTIDAS, caminho).then(function(){ return valor; });
  });
}

function resolverAcaoUnicaFatia1a(pid, match, faseNum, time, pos){
  var acoesFase = (match.fases[faseNum] && match.fases[faseNum].acoes) || {};
  var ac = acoesFase[time + '_' + pos];
  if(!ac || ac.resolvida) return Promise.resolve('nao_aplicavel');

  var ACOES_DESTA_FATIA = ['rastrear','distracao','passar','lancar_goles','auxiliar','inspirar','provocar','sabotar','carregar'];
  if(ACOES_DESTA_FATIA.indexOf(ac.acao) === -1) return Promise.resolve('nao_aplicavel');

  function sl(t, p){ return match.times[t].slots[p] || {}; }
  function advT(t){ return t === 'A' ? 'B' : 'A'; }
  var slotAtor = sl(time, pos);

  var logIndex = logSequencialProxIndex(match, faseNum);
  var entradasLog = [];
  var savesExtra = [];

  function log(txt, extra){
    entradasLog.push(Object.assign({ text: txt }, extra || {}));
  }
  ...
  function finalizar(){
    ...
    if(pos === 'apanhador' && QUAD_APANHADOR_ACOES_CACA.indexOf(ac.acao) !== -1){
      var novoCaca = (slotAtor.cacaUsados || 0) + 1;
      saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + time + '/slots/apanhador', { cacaUsados: novoCaca }));
    }

    return Promise.all(saves).then(function(){ return 'resolvida'; });
  }

  switch(ac.acao){

    case 'rastrear': {
      var distraidoFlag = ((match.fases[faseNum] || {}).distraido || {})[time];
      if(distraidoFlag){
        log(slotAtor.nome + ' tentou rastrear, mas estava distraído(a) e não conseguiu.');
        return finalizar();
      }
      var protegidoVig = !!((match.fases[faseNum] || {}).protegidos || {})[time + '_apanhador'];
      var incremento = protegidoVig ? 2 : 1;
      var novoRastreio = (slotAtor.rastreamento || 0) + incremento;
      savesExtra.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + time + '/slots/apanhador', { rastreamento: novoRastreio }));
      if(protegidoVig){
        savesExtra.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_COMBO));
        savesExtra.push(somarMvpSeq(pid, time + '_apanhador', QUAD_MVP_COMBO));
        log('Combo: Vigilância. ' + slotAtor.nome + ' ganha rastreio extra por estar protegido(a) pelo Batedor.');
      }
      log(slotAtor.nome + ' rastreou o Pomo. Bônus acumulado: +' + novoRastreio + '.');
      return finalizar();
    }

    case 'distracao': {
      savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/distraido/' + advT(time), true));
      savesExtra.push(somarMvpSeq(pid, time + '_apanhador', QUAD_MVP_APANHADOR_DISTRACAO));
      log(slotAtor.nome + ' fez uma Distração. ' + sl(advT(time), 'apanhador').nome + ' vai perder a fase se tentar rastrear.', { afeta: [advT(time) + '_apanhador'] });
      return finalizar();
    }
    ...
    case 'passar': {
      ...
      log(slotAtor.nome + ' passou a Goles para ' + sl(time, ac.alvo).nome + '. +2 no próximo arremesso.', { afeta: [time + '_' + ac.alvo] });
      return finalizar();
    }
...
case 'lancar_goles':
    ...
    case 'provocar': {
      if(!ac.alvo){ return finalizar(); }
      var alvoProvocado = sl(advT(time), ac.alvo);
      savesExtra.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + advT(time) + '/slots/' + ac.alvo, {
        debuff: { atributo: MAIN_ATTR[ac.alvo] || 'agilidade', valor:-2, fases:1, faseAplicada: faseNum }
      }));
      log(slotAtor.nome + ' provocou ' + alvoProvocado.nome + '. -2 no atributo principal por 1 fase.', { afeta: [advT(time) + '_' + ac.alvo] });
      return finalizar();
    }

case 'sabotar': {
      return consumirBonusSituacionalSeq(pid, faseNum, time, pos).then(function(bonusAcumulado){
        ...
        if(vencedor(rAtk, rDef)){
          ...
          log(slotAtor.nome + ' sabotou ' + advAph.nome + '. Rastreamento zerado.', { afeta: [advT(time) + '_apanhador'], dados: dadosSab });
        }else{
          log(slotAtor.nome + ' tentou sabotar, mas falhou.', { dados: dadosSab });
        }
        return finalizar();
      });
    }
...
var dadosCar = { atkNome: slotAtor.nome, atkDado: rAtk2.dado, atkBonus: rAtk2.bonus, atkTotal: rAtk2.total, atkDetalhe: rAtk2.detalhe, defNome: advAph2.nome, defDado: rDef2.dado, defBonus: rDef2.bonus, defTotal: rDef2.total, defDetalhe: rDef2.detalhe };
        if(vencedor(rAtk2, rDef2)){
          ...
          savesExtra.push(somarMvpSeq(pid, time + '_apanhador', QUAD_MVP_APANHADOR_CARREGAR));
          log(slotAtor.nome + ' derrubou ' + advAph2.nome + '! Rastreamento zerado, ele não age nesta fase.', { afeta: [advT(time) + '_apanhador'], dados: dadosCar });
          if(advAph2.uid && advAph2.uid !== 'npc') savesExtra.push(deduzirHp(advAph2.uid, QUAD_HP_DANO));
        }else{
          log(slotAtor.nome + ' tentou derrubar ' + advAph2.nome + ', mas foi esquivado.', { dados: dadosCar });
        }
        return finalizar();
      });
    }
    ...
    default:
      return Promise.resolve('nao_aplicavel');
  }
}


function montarOpcoesRollSeq(match, faseNum, time, pos, tipoVassoura, especialidade, acaoObj, situacional){
...
function resolverAcaoUnicaFatia1b(pid, match, faseNum, time, pos){
  ...
  var ACOES_DESTA_FATIA = ['fintar','mergulho','finta_passe','antecipar','interceptar','derrubar_artilheiro','blitz'];
  if(ACOES_DESTA_FATIA.indexOf(ac.acao) === -1) return Promise.resolve('nao_aplicavel');

  function sl(t, p){ return match.times[t].slots[p] || {}; }
  function advT(t){ return t === 'A' ? 'B' : 'A'; }
  var slotAtor = sl(time, pos);
  var advTime = advT(time);

  var logIndex = logSequencialProxIndex(match, faseNum);
  var entradasLog = [];
  var savesExtra = [];

  function log(txt, extra){ entradasLog.push(Object.assign({ text: txt }, extra || {})); }
 ...
 case 'finta_passe': {
      ...
      if(vencedor(rAtkFP, rDefFP)){
        ...
        log(slotAtor.nome + ' enganou o Goleiro com uma Finta de Passe. -3 na defesa dele nesta fase.', { afeta: [advTime + '_goleiro'], dados: dadosFP });
      }else{
        log(slotAtor.nome + ' tentou uma Finta de Passe, mas foi lido.', { dados: dadosFP });
      }
      return finalizar();
    }
...
case 'antecipar': {
      ...
      var dadosAnt = { atkNome: advArt.nome, atkDado: rAtkAnt.dado, atkBonus: rAtkAnt.bonus, atkTotal: rAtkAnt.total, atkDetalhe: rAtkAnt.detalhe, defNome: slotAtor.nome, defDado: rDefAnt.dado, defBonus: rDefAnt.bonus, defTotal: rDefAnt.total, defDetalhe: rDefAnt.detalhe };
      if(vencedor(rDefAnt, rAtkAnt)){
        ...
        log(slotAtor.nome + ' antecipou o jogo. +5 na defesa desta fase.', { dados: dadosAnt });
      }else{
        log(slotAtor.nome + ' tentou antecipar, mas não conseguiu.', { dados: dadosAnt });
      }
      return finalizar().then(function(resultado){
        return resolverPendentesContraGoleiro(pid, match, faseNum, time).then(function(){ return resultado; });
      });
    ...
    case 'mergulho': {
      var gsMer = sl(advTime, 'goleiro');
      var protegidoMer = !!((match.fases[faseNum] || {}).protegidos || {})[time + '_' + pos];
...
if(vencedor(rAtkMer, rDefMer)){
    ...
    case 'fintar': {
      ...
      if(vencedor(rAtkFin, rDefFin)){
        ...
        savesExtra.push(somarMvpSeq(pid, time + '_' + pos, QUAD_MVP_CONTROLE));
        log(slotAtor.nome + ' executou uma finta perfeita. Imune ao Balaço esta fase.', { dados: dadosFin });
      }else{
        log(slotAtor.nome + ' tentou fintar, mas o Batedor previu.', { dados: dadosFin });
      }
      return finalizar();
    }

    case 'derrubar_artilheiro': {
      function finalizarComLiberacao(){
        return finalizar().then(function(resultado){
          return resolverPendentesContraGoleiro(pid, match, faseNum, time).then(function(){ return resultado; });
        });
      }
      if(!ac.alvo) return finalizarComLiberacao();
      ...
      if(vencedor(rAtkDA, rDefDA)){
        ...
        log(slotAtor.nome + ' derrubou ' + alsDA.nome + ' no ar! Não vai arremessar esta fase. -' + QUAD_HP_DANO + ' HP.', { afeta: [advTime + '_' + ac.alvo], dados: dadosDA });
        if(alsDA.uid && alsDA.uid !== 'npc') savesExtra.push(deduzirHp(alsDA.uid, QUAD_HP_DANO));
      }else{
        log(slotAtor.nome + ' tentou derrubar ' + alsDA.nome + ', mas errou.', { dados: dadosDA });
      }
      return finalizarComLiberacao();
    }
    ...
    case 'interceptar': {
      ...
      if(vencedor(rAtkInt, rDefInt)){
        ...
        log(slotAtor.nome + ' interceptou o passe adversário! Artilheiros adversários perdem o bônus de Passar a partir de agora.', { dados: dadosInt });
      }else{
        log(slotAtor.nome + ' tentou interceptar, mas falhou.', { dados: dadosInt });
      }
      return finalizar();
    }
...
case 'blitz': {
      ...
      if(vencedorOuIgual(rDefBli, rAtkBli)){
        ...
        log(slotAtor.nome + ' executou o Blitz com sucesso! Interceptou a Goles.', { dados: dadosBli });
      }else{
        savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/blitzResult/' + time, 'fail'));
        log(slotAtor.nome + ' tentou o Blitz e falhou! Os aros ficaram desprotegidos.', { dados: dadosBli });
      }
      return finalizar().then(function(resultado){
        return resolverPendentesContraGoleiro(pid, match, faseNum, time).then(function(){ return resultado; });
      });
...
function pontosGolSeq(match, faseNum, time){
...
function resolverArremessoUnico(pid, match, faseNum, time, pos){
  ...
  var acoesFase = faseObj.acoes || {};
  var ac = acoesFase[time + '_' + pos];
  if(!ac || ac.acao !== 'arremessar') return Promise.resolve('nao_aplicavel');

  function sl(t, p){ return match.times[t].slots[p] || {}; }
  function advT(t){ return t === 'A' ? 'B' : 'A'; }
  var ast = sl(time, pos);
  var golvT = advT(time);
  var gs = sl(golvT, 'goleiro');
  ...
  var derrubados = faseObj.derrubados || {};
  if(derrubados[time + '_' + pos]){
    log(ast.nome + ' estava caído e não conseguiu arremessar.');
    return finalizar();
  }

  var blitzResult = faseObj.blitzResult || {};
...
var passeBonus = ((faseObj.bonusSituacional || {})[time + '_' + pos]) || 0;
  ...
  if(protegidoAtk){
    savesExtra.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_COMBO));
    savesExtra.push(somarMvpSeq(pid, time + '_' + pos, QUAD_MVP_COMBO));
    log('Combo: Cobertura Perfeita. ' + ast.nome + ' arremessa protegido pelo Batedor, bônus extra.');
  }
  var situAtk = passeBonus + mergulhoBonus + (protegidoAtk ? 3 : 0);

  var acaoGoleiro = acoesFase[golvT + '_goleiro'];
  var situDef = (acaoGoleiro && acaoGoleiro.acao === 'defender' ? 2 : 0)
    + ((faseObj.antecipeBonus || {})[golvT] || 0)
    - ((faseObj.fintaPasseDebuff || {})[golvT] || 0)
    + (expostoAtk ? 2 : 0);

  var rAtk = rolar(ast, 'destreza', 'agilidade', montarOpcoesRollSeq(match, faseNum, time, pos, 'velocidade', 'artilheiro', ac, situAtk));
...
function resolverPendentesContraGoleiro(pid, match, faseNum, golvTime){
  return fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/pendentes').then(function(pendentes){
    pendentes = pendentes || {};
    var chaves = Object.keys(pendentes).filter(function(chave){
      return pendentes[chave] && pendentes[chave].tipo === 'arremessar' && pendentes[chave].golvTime === golvTime;
    });
    if(!chaves.length) return Promise.resolve();

return chaves.reduce(function(p, chave){
      return p.then(function(){
        return fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(matchFresco){
          matchFresco = matchFresco || match;
          var partes = chave.split('_');
          var timeAtacante = partes[0], posAtacante = partes.slice(1).join('_');
          var golvCaidoAgora = !!((matchFresco.fases[faseNum] || {}).derrubados || {})[golvTime + '_goleiro'];
          if(golvCaidoAgora){
            return resolverArremessoContraGoleiroCaido(pid, matchFresco, faseNum, timeAtacante, posAtacante, golvTime);
          }
          return resolverArremessoUnico(pid, matchFresco, faseNum, timeAtacante, posAtacante);
        });
      });
    }, Promise.resolve());
  });
...
function resolverAcaoUnicaFatia1c(pid, match, faseNum, time, pos){
  var acoesFase = (match.fases[faseNum] && match.fases[faseNum].acoes) || {};
  var ac = acoesFase[time + '_' + pos];
  if(!ac || ac.resolvida) return Promise.resolve('nao_aplicavel');

  var ACOES_DESTA_FATIA = ['proteger','lancar_balaco','derrubar','pressao_aerea','defender','arremessar'];
  if(ACOES_DESTA_FATIA.indexOf(ac.acao) === -1) return Promise.resolve('nao_aplicavel');
  ...
  switch(ac.acao){
    ...
    case 'proteger': {
      if(!ac.alvo) return finalizar();
      savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/protegidos/' + time + '_' + ac.alvo, true));
      log(slotAtor.nome + ' protegeu ' + sl(time, ac.alvo).nome + '.', { afeta: [time + '_' + ac.alvo] });
      return finalizar();
    }

    case 'lancar_balaco': {
      if(!ac.alvo) return finalizar();
      var alsLB = sl(advTime, ac.alvo);
      var alvKeyLB = advTime + '_' + ac.alvo;
      var faseObjLB = match.fases[faseNum] || {};

      if((faseObjLB.fintouB || {})[alvKeyLB]){
        log(slotAtor.nome + ' lançou o Balaço em ' + alsLB.nome + ', mas ele esquivou.');
        return finalizar();
      }
      if((faseObjLB.protegidos || {})[alvKeyLB]){
        log(slotAtor.nome + ' lançou o Balaço em ' + alsLB.nome + ', mas estava protegido.');
        return finalizar();
      }
...
var rAtkLB = rolar(slotAtor, 'forca', 'determinacao', montarOpcoesRollSeq(match, faseNum, time, 'batedor', 'velocidade', 'manobras', ac));
      ...
      if(vencedor(rAtkLB, rDefLB)){
        ...
        log(slotAtor.nome + ' acertou o Balaço em ' + alsLB.nome + '! -2 em ' + (MAIN_ATTR[ac.alvo] || 'agilidade') + ' e -' + QUAD_HP_DANO + ' HP.', { afeta: [alvKeyLB], dados: dadosLB });
        if(alsLB.uid && alsLB.uid !== 'npc') savesExtra.push(deduzirHp(alsLB.uid, QUAD_HP_DANO));
      }else{
        log(slotAtor.nome + ' lançou o Balaço em ' + alsLB.nome + ', mas errou.', { dados: dadosLB });
      }
      return finalizar();
    }
      ...
      if(vencedor(rAtkD, rDefD)){
        savesExtra.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/derrubados/' + advTime + '_' + ac.alvo, true));
        savesExtra.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_DEBUFF));
        log(slotAtor.nome + ' derrubou ' + alsD.nome + ' em colisão! Não age nesta fase. -' + QUAD_HP_DANO + ' HP.', { afeta: [advTime + '_' + ac.alvo], dados: dadosD });
        if(alsD.uid && alsD.uid !== 'npc') savesExtra.push(deduzirHp(alsD.uid, QUAD_HP_DANO));
        if(ac.alvo === 'goleiro'){
          return finalizar().then(function(resultado){
            return resolverPendentesContraGoleiro(pid, match, faseNum, advTime).then(function(){ return resultado; });
          });
        }
      }else{
        log(slotAtor.nome + ' tentou derrubar ' + alsD.nome + ', mas foi evitado.', { dados: dadosD });
      }
      return finalizar();
    }
    ...
    case 'pressao_aerea': {
      ...
      if(vencedor(rAtkPA, rDefPA)){
        ...
        savesExtra.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_DEBUFF));
        log(slotAtor.nome + ' intimidou ' + alsPA.nome + ' no ar. -2 no próximo roll dele.', { afeta: [advTime + '_' + ac.alvo], dados: dadosPA });
      }else{
        log(slotAtor.nome + ' tentou intimidar ' + alsPA.nome + ', sem efeito.', { dados: dadosPA });
      }
      return finalizar();
    }
...
case 'defender': {
    ...
    case 'arremessar': {
      var golvT = advTime;
      var acGoleiro = (match.fases[faseNum].acoes || {})[golvT + '_goleiro'];
      var goleiroJaResolveu = acGoleiro && acGoleiro.resolvida;
...
log(slotAtor.nome + ' está arremessando a goles.', { afeta: [time + '_' + pos] });
        ...
        return Promise.all(saves).then(function(){ return 'pendente'; });
      }

var golvCaidoAgora = !!((match.fases[faseNum] || {}).derrubados || {})[golvT + '_goleiro'];
      if(golvCaidoAgora){
        return resolverArremessoContraGoleiroCaido(pid, match, faseNum, time, pos, golvT);
      }

      return resolverArremessoUnico(pid, match, faseNum, time, pos);
    }
...
var QUAD_GOLEIRO_LEVANTAR_DC = 14;

function resolverArremessoContraGoleiroCaido(pid, match, faseNum, time, pos, golvT){
  function sl(t, p){ return match.times[t].slots[p] || {}; }
  var ast = sl(time, pos);
  var gs = sl(golvT, 'goleiro');
  var faseObj = match.fases[faseNum] || {};
  var golsCaidoUsados = ((faseObj.golsGoleiroCaido || {})[golvT]) || 0;

  var logIndex = logSequencialProxIndex(match, faseNum);
  var entradas = [];
  function log(txt){ entradas.push({ text: txt }); }

  function fecharComGol(pts, marcadorGols){
    var saves = [];
  ...
  if(golsCaidoUsados === 0){
    var pts1 = pontosGolSeq(match, faseNum, time);
    log(gs.nome + ' perdeu o primeiro gol por estar caído(a).');
    log(ast.nome + ' arremessou sem oposição. GOOOOL! +' + pts1 + ' para ' + match.times[time].nome + '.');
    return fecharComGol(pts1, 1);
  }

  var dadoLevantar = 1 + Math.floor(Math.random() * 20);
  var bonusDet = modAtributo(gs, 'determinacao', 2);
  var totalLevantar = dadoLevantar + bonusDet;
  var conseguiu = totalLevantar >= QUAD_GOLEIRO_LEVANTAR_DC;

  log(gs.nome + ' tenta se levantar para defender o segundo gol... (dado ' + dadoLevantar + ' + ' + bonusDet + ' = ' + totalLevantar + ', precisa de ' + QUAD_GOLEIRO_LEVANTAR_DC + ' ou mais)');
...
log(gs.nome + ' conseguiu se levantar! Vai defender tentar o defender o novo arremesso.');
  ...
  return Promise.all(saves2).then(function(){
    return fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid).then(function(matchFresco){
      return resolverArremessoUnico(pid, matchFresco || match, faseNum, time, pos);
    });
  });
}

function resolverComboCombinado(pid, match, faseNum, time, comboPendente, posCompletou, acCompletou){
  function sl(t, p){ return match.times[t].slots[p] || {}; }
  function advT(t){ return t === 'A' ? 'B' : 'A'; }
  var advTime = advT(time);

  var acGoleiro = comboPendente.pos === 'goleiro' ? { acao: comboPendente.acao } : acCompletou;
  var acBatedor = comboPendente.pos === 'batedor' ? { acao: comboPendente.acao } : acCompletou;
  var tipoCombo = (acGoleiro.acao === 'antecipar') ? 'muralha' : 'cerco';

  var gs = sl(time, 'goleiro'), bs = sl(time, 'batedor');
  var advArt = sl(advTime, 'artilheiro1');

  var logIndex = logSequencialProxIndex(match, faseNum);
  var entradas = [];
  var saves = [];
  function log(txt, extra){ entradas.push(Object.assign({ text: txt }, extra || {})); }

  var rGol, rBat, rDef, sucesso;

 if(tipoCombo === 'muralha'){
    rGol = rolar(gs, 'inteligencia', 'agilidade', montarOpcoesRollSeq(match, faseNum, time, 'goleiro', 'estabilidade', 'defesa', acGoleiro));
    ...
    if(sucesso){
      ...
      saves.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/interceptados/' + advTime, true));
      saves.push(somarMvpSeq(pid, time + '_goleiro', QUAD_MVP_COMBO));
      saves.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_COMBO));
      log('Combo: Muralha. ' + gs.nome + ' e ' + bs.nome + ' formam uma defesa perfeita.', { combo:true, nome:'Muralha', dados: dadosComboM });
    }else{
      log(gs.nome + ' e ' + bs.nome + ' tentaram a Muralha, mas falharam.', { dados: dadosComboM });
    }
  }else{
    rGol = rolar(gs, 'forca', 'agilidade', montarOpcoesRollSeq(match, faseNum, time, 'goleiro', 'velocidade', 'defesa', acGoleiro));
    rBat = rolar(bs, 'forca', 'determinacao', montarOpcoesRollSeq(match, faseNum, time, 'batedor', 'bonus', 'manobras', acBatedor));
    ...
    if(sucesso){
      ...
      saves.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/interceptados/' + advTime, true));
      saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/times/' + advTime + '/slots/artilheiro1', {
        debuff: { atributo: MAIN_ATTR.artilheiro1, valor:-2, fases:9999, permanente:true }
      }));
      saves.push(somarMvpSeq(pid, time + '_goleiro', QUAD_MVP_COMBO));
      saves.push(somarMvpSeq(pid, time + '_batedor', QUAD_MVP_COMBO));
      log('Combo: Cerco. ' + gs.nome + ' e ' + bs.nome + ' encurralam ' + advArt.nome + '. Debuff permanente aplicado.', { combo:true, nome:'Cerco', dados: dadosComboC });
    }else{
      saves.push(fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/blitzResult/' + time, 'fail'));
      log(gs.nome + ' e ' + bs.nome + ' tentaram o Cerco, mas falharam.', { dados: dadosComboC });
    }
  }
  ...
  saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/acoes/' + time + '_goleiro', { resolvida: true }));
  saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/acoes/' + time + '_batedor', { resolvida: true }));
  saves.push(fbDel(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/comboPendente/' + time));

  return Promise.all(saves).then(function(){
    return resolverPendentesContraGoleiro(pid, match, faseNum, time);
  }).then(function(){ return 'combo_resolvido'; });
}

 
function resolverComboPendenteSolo(pid, match, faseNum, time, comboPendente){
  return fbDel(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/comboPendente/' + time).then(function(){
    return resolverAcaoUnicaFatia1b(pid, match, faseNum, time, comboPendente.pos);
  });
}

function resolverAcaoUnicaSequencial(pid, match, faseNum, time, pos){
  var acoesFase = (match.fases[faseNum] && match.fases[faseNum].acoes) || {};
  var ac = acoesFase[time + '_' + pos];
  if(!ac || ac.resolvida) return Promise.resolve('nao_aplicavel');
...
var derrubadosAtual = ((match.fases[faseNum] || {}).derrubados) || {};
  ...
  function cadeiaNormal(){
    return resolverAcaoUnicaFatia1a(pid, match, faseNum, time, pos).then(function(r){
      if(r !== 'nao_aplicavel') return r;
      return resolverAcaoUnicaFatia1b(pid, match, faseNum, time, pos);
    }).then(function(r){
      if(r !== 'nao_aplicavel') return r;
      return resolverAcaoUnicaFatia1c(pid, match, faseNum, time, pos);
    });
  }

  if(pos !== 'goleiro' && pos !== 'batedor'){
    return cadeiaNormal();
  }

  var parceiroPos = pos === 'goleiro' ? 'batedor' : 'goleiro';
  var comboPendente = ((match.fases[faseNum] || {}).comboPendente || {})[time];
  var ACOES_COMBO = { goleiro: ['antecipar','blitz'], batedor: ['interceptar'] };
  var minhaAcaoECandidata = ACOES_COMBO[pos].indexOf(ac.acao) !== -1;

  if(comboPendente && comboPendente.pos === parceiroPos){
    var combina =
      (comboPendente.acao === 'antecipar' && ac.acao === 'interceptar') ||
      (comboPendente.acao === 'blitz' && ac.acao === 'interceptar') ||
      (comboPendente.acao === 'interceptar' && (ac.acao === 'antecipar' || ac.acao === 'blitz'));

    if(combina){
      return resolverComboCombinado(pid, match, faseNum, time, comboPendente, pos, ac);
    }
    return resolverComboPendenteSolo(pid, match, faseNum, time, comboPendente).then(function(){
      return cadeiaNormal();
    });
  }

  if(minhaAcaoECandidata){
    var parceiroJaAgiu = acoesFase[time + '_' + parceiroPos];
    if(!parceiroJaAgiu){
      var logIndex = logSequencialProxIndex(match, faseNum);
      var saves = [
        fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/comboPendente/' + time, { pos: pos, acao: ac.acao, ts: Date.now() }),
        fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + faseNum + '/resultado/log/' + logIndex, {
          text: match.times[time].slots[pos].nome + ' está tentando um combo, aguardando o parceiro.',
          apenasTime: time
        })
      ];
      return Promise.all(saves).then(function(){ return 'pendente_combo'; });
    }
  }
...
function resolverDisputaForcadaPomoSeq(pid, match, faseNum){
  if(match.pomo_capturado) return Promise.resolve();
  if(faseNum < (match.pomo_fase || QUAD_POMO_MIN)) return Promise.resolve();

  function sl(t, p){ return match.times[t].slots[p] || {}; }

  var derrubados = ((match.fases[faseNum] || {}).derrubados) || {};
  var aphACaido = !!derrubados['A_apanhador'];
  var aphBCaido = !!derrubados['B_apanhador'];
...
var logIndex = logSequencialProxIndex(match, faseNum);
  ...
  if(aphACaido && !aphBCaido){
    ...
    if(aphB.uid && aphB.uid !== 'npc') saves.push(deduzirHp(aphB.uid, QUAD_HP_DANO));

    if(vencedor(rA, rB)){
      pomoBonusNovo = { time:'A' };
      log(aphA.nome + ' capturou o Pomo de Ouro! ' + match.times.A.nome + ' ganha a janela de gol em dobro.', { dados: dadosPomo });
    }else if(vencedor(rB, rA)){
      pomoBonusNovo = { time:'B' };
      log(aphB.nome + ' capturou o Pomo de Ouro! ' + match.times.B.nome + ' ganha a janela de gol em dobro.', { dados: dadosPomo });
    }else{
      var rastA = aphA.rastreamento || 0, rastB = aphB.rastreamento || 0;
      if(rastA > rastB){
        pomoBonusNovo = { time:'A' };
        log('Empate na disputa! ' + aphA.nome + ' vence no desempate por ter mais rastreio (' + rastA + ' vs ' + rastB + ').', { dados: dadosPomo });
      }else if(rastB > rastA){
        pomoBonusNovo = { time:'B' };
        log('Empate na disputa! ' + aphB.nome + ' vence no desempate por ter mais rastreio (' + rastB + ' vs ' + rastA + ').', { dados: dadosPomo });
      }else{
        pomoBonusNovo = { time: Math.random() < 0.5 ? 'A' : 'B' };
        log('Empate total na disputa (mesmo rastreio)! ' + match.times[pomoBonusNovo.time].nome + ' venceu no sorteio.', { dados: dadosPomo });
      }
    }
  }

  if(pomoBonusNovo && entradas.length){
    entradas[entradas.length - 1].pomo = true;
    entradas[entradas.length - 1].nome = 'Pomo Capturado';
  }
  ...
  if(pomoBonusNovo){
    saves.push(fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid, {
      pomo_capturado: true,
      pomo_bonus: { time: pomoBonusNovo.time, ate_fase: faseNum + QUAD_POMO_BONUS_FASES }
    }));
  }
...
function finalizarPartidaSequencial(pid, match){
  ...
  return fbGet(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/pontuadores').then(function(pontuadores){
    pontuadores = pontuadores || {};
    ...
    var mvpNome = null;
    if(mvpSk){
      var partes = mvpSk.split('_');
      var slotMvp = match.times[partes[0]].slots[partes.slice(1).join('_')];
    ...
    return Promise.all([
      fbPatch(QUAD_FB_PARTIDAS, '/partidas/' + pid, { status:'encerrada', vencedor: vencedorFinal }),
      fbPut(QUAD_FB_PARTIDAS, '/historico/' + pid, {
        data: Date.now(),
...
var QUAD_PAUSA_ENTRE_FASES_MS = 10000;
...
if(!eventoAindaAtivo){
            ...
            if(eventoNovo){
              patchMatch.evento_ativo = { id: eventoNovo.id, nome: eventoNovo.nome, ativo: eventoNovo.ativo, afeta: eventoNovo.afeta, penalidade: eventoNovo.penalidade, desde_fase: nextFase, ate_fase: nextFase + QUAD_EVENTO_DURACAO_FASES };
...
function renderLogAbas(idTabs, idConteudo, fasesObj, mapaCores){
...
if(!logAbasEstado[idTabs]) logAbasEstado[idTabs] = { abaAtual: numeros[numeros.length - 1], contagens: {}, maiorFaseConhecida: numeros[numeros.length - 1] };
  ...
  function pegarLogFase(n){
    return (fasesObj[n] && fasesObj[n].resultado && fasesObj[n].resultado.log) || [];
  }

function renderConteudoAba(){
    var n = estado.abaAtual;
    var logFase = pegarLogFase(n);
    var contagemAnterior = estado.contagens[n] || 0;

    if(logFase.length === contagemAnterior && elConteudo.dataset.faseAtual === String(n)) return;

    var estavaNoFim = (elConteudo.scrollHeight - elConteudo.scrollTop - elConteudo.clientHeight) < 20;

    estado.contagens[n] = logFase.length;
    elConteudo.dataset.faseAtual = String(n);

    elConteudo.innerHTML = logFase.map(function(entrada){
      var cls = 'q3logl' + (entrada.combo ? ' q3logcombo' : '') + (entrada.pomo ? ' q3logpomo' : '');
      return '<div class="' + cls + '">' + colorirNomesNoLog(escaparHtml(entrada.text || ''), mapaCores) + spoilerDadosHtml(entrada.dados) + '</div>';
    }).join('');

    if(estavaNoFim){
      elConteudo.scrollTop = elConteudo.scrollHeight;
    }
  }

  var chaveAtualTabs = numeros.map(function(n){ return n + ':' + pegarLogFase(n).length; }).join(',');

  if(elTabs.dataset.chave !== chaveAtualTabs){
    elTabs.dataset.chave = chaveAtualTabs;
    elTabs.innerHTML = numeros.map(function(n){
      var temPomo = pegarLogFase(n).some(function(e){ return e && e.pomo; });
      return '<button type="button" class="q3logababtn' + (temPomo ? ' q3logabapomo' : '') + (n === estado.abaAtual ? ' on' : '') + '" data-fase="' + n + '">Fase ' + n + '</button>';
    }).join('');

    Array.prototype.forEach.call(elTabs.querySelectorAll('.q3logababtn'), function(btn){
      btn.onclick = function(){
        estado.abaAtual = parseInt(btn.dataset.fase, 10);
        Array.prototype.forEach.call(elTabs.querySelectorAll('.q3logababtn'), function(b){ b.classList.remove('on'); });
        btn.classList.add('on');
        renderConteudoAba();
      };
    });
    ...
    Array.prototype.forEach.call(elTabs.querySelectorAll('.q3logababtn'), function(btn){
      btn.classList.toggle('on', parseInt(btn.dataset.fase, 10) === estado.abaAtual);
    });
  }
...
function filtrarLogJogador(logObjFase, meuTime, meuPos){
  var minhaChave = meuTime + '_' + meuPos;
  var chaves = Object.keys(logObjFase || {}).map(Number).sort(function(a, b){ return a - b; });

  return chaves.map(function(k){ return logObjFase[k]; }).filter(function(entrada){
    if(entrada.afeta && entrada.afeta.indexOf(minhaChave) !== -1) return true;
    if(entrada.apenasTime === meuTime && (meuPos === 'goleiro' || meuPos === 'batedor')) return true;
    return false;
}).map(function(entrada){
    return {
      texto: entrada.text,
      destaque: !!entrada.apenasTime,
      combo: !!entrada.combo,
      dados: entrada.dados
    };
  });
...
function renderEscolhaOrdem(match){
  ...
  var elForcar = document.getElementById('q3ordemforcar');
  if(isAdmin && (!okA || !okB)){
    elForcar.hidden = false;
    elForcar.onclick = function(){ forcarOrdemPadrao(fase); };
  }else{
    elForcar.hidden = true;
  }
}
...
function forcarOrdemPadrao(fase){
  if(!confirm('Forçar ordem padrão pra quem ainda não respondeu nesta fase?')) return;
      ...
      if(Date.now() >= pend.deadline){
        fbPut(QUAD_FB_PARTIDAS, '/partidas/' + pid + '/fases/' + fase + '/acoes/' + sk, escolherAcaoNpc(atual.pos))
          .then(function(){
            ...
            if(!matchAtual) return Promise.resolve();
            return resolverAcaoUnicaSequencial(pid, matchAtual, fase, atual.time, atual.pos);
          }).then(avancarTurno);
      }
      return;
    }
...
function gerarPainelCapitao(match, time){
  ...
  html += '<div class="q3acoesgrid">';
html += '<button type="button" class="q3capacaobtn" data-time="' + time + '" data-tipo="motivar"><i class="fa fa-bolt"></i> Motivar</button>';
  html += '<button type="button" class="q3capacaobtn" data-time="' + time + '" data-tipo="estrategia"><i class="fa fa-magic"></i> Estratégia</button>';
  html += '</div>';
...
function corPrincipalCasa(nome){
  if(!nome) return null;
  if(QUAD_CORES_CASAS_DESTAQUE[nome]) return QUAD_CORES_CASAS_DESTAQUE[nome];
  var alvo = nome.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  var chave = Object.keys(QUAD_CORES_CASAS_DESTAQUE).find(function(k){
    return k.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase() === alvo;
  });
  return chave ? QUAD_CORES_CASAS_DESTAQUE[chave] : null;
}

function mapaCoresNomesJogadores(match){
  var mapa = {};
  ['A','B'].forEach(function(t){
    var cor = corPrincipalCasa(match.times[t].nome);
    if(!cor) return;
    POSICOES.forEach(function(pos){
      var slot = match.times[t].slots[pos];
      if(slot && slot.nome) mapa[slot.nome] = cor;
    });
  });
  return mapa;
}

function colorirNomesNoLog(textoEscapado, mapaCores){
  if(!mapaCores) return textoEscapado;
  var nomes = Object.keys(mapaCores).sort(function(a, b){ return b.length - a.length; });
  nomes.forEach(function(nome){
    var nomeEscapado = escaparHtml(nome);
    if(!nomeEscapado || textoEscapado.indexOf(nomeEscapado) === -1) return;
    textoEscapado = textoEscapado.split(nomeEscapado).join(
      '<span style="color:' + mapaCores[nome] + ';font-weight:600;">' + nomeEscapado + '</span>'
    );
  });
...
function tocarVuvuzela(nome){
  ...
  if(sons){
    ...
    try{
      var a = new Audio(url); a.volume = 0.25;
      var p = a.play();
...
function montarSlotTreino(uid){
  return Promise.all([
    ...
    return {
      atributos: atributos,
      vassoura: { nome:'Vassoura Padrao de Hogwarts', bonus:0 }, 
      maestria: {
        quadribol: pctQuadribol,
        voo: 0,
        artilheiro: (cadastro.especializacoes && cadastro.especializacoes.artilheiro) || 0,
...
document.getElementById('q5combuscar').onclick = function(){
    ...
    if(!uidRaw){ alert('Informe o uid.'); return; }
    var uid = uidRaw.charAt(0) === 'u' ? uidRaw : ('u' + uidRaw);
...
function criarPartidaDoAdmin(){
  ...
  var dataAgendada = document.getElementById('q5partidadata').value || null;
  var horaAgendada = document.getElementById('q5partidahora').value || null;
      ...
      POSICOES.forEach(function(pos){
        ...
        if(cad.isNpc){
          slots[pos] = {
            ...
            vassoura: cad.vassoura || { nome:'Vassoura Padrao de Hogwarts', bonus:0 },
            maestria: {
              quadribol: cad.maestriaPct || 0,
              artilheiro: (cad.especializacoes && cad.especializacoes.artilheiro) || 0,
              defesa: (cad.especializacoes && cad.especializacoes.defesa) || 0,
...
var PFATRIB_ICONS = {
  forca:'gi-muscle-up', resistencia:'gi-shield', agilidade:'gi-sprint', destreza:'gi-juggler',
  sabedoria:'gi-scroll-unfurled', carisma:'gi-charm', inteligencia:'gi-brain', determinacao:'gi-fist'
};
var PFATRIB_MAX = 25;
var PFATRIB_CIRC = 188.5;

function anelAtributoHtml(nome, valor){
  valor = (valor === undefined || valor === null) ? 0 : valor;
  var pct = Math.min(1, valor / PFATRIB_MAX);
  var offset = (PFATRIB_CIRC * (1 - pct)).toFixed(1);
  var valFormatado = String(valor).padStart(2, '0');
  return '<div class="pfatrib-item">' +
    '<div class="pfatrib-ring"><svg viewBox="0 0 72 72">' +
      '<circle class="track" cx="36" cy="36" r="30"></circle>' +
      '<circle class="fill" cx="36" cy="36" r="30" stroke-dasharray="' + PFATRIB_CIRC + '" stroke-dashoffset="' + offset + '"></circle>' +
    '</svg>' +
    '<div class="pfatrib-center"><span class="pfatrib-val">' + valFormatado + '</span><i class="gi ' + (PFATRIB_ICONS[nome] || '') + ' pfatrib-icon"></i></div>' +
    '</div>' +
    '<span class="pfatrib-nome">' + ATTR_LABEL[nome] + '</span>' +
  '</div>';
}
...
var attrsBox = document.getElementById('q3fichaattrs');
  ...
  attrsBox.className = 'pfatributos';
  var corCasaFicha = corPrincipalCasa(match.times[time].nome);
  if(corCasaFicha) attrsBox.style.setProperty('--casa-stroke', corCasaFicha);
  attrsBox.innerHTML = Object.keys(ATTR_LABEL).map(function(a){
    return anelAtributoHtml(a, atributos[a]);
  }).join('');

var maest = slot.maestria || {};
  document.getElementById('q3fichamaest').innerHTML =
    '<span class="q3fichamaestitem">Quadribol: ' + (maest.quadribol || 0) + '%</span>' +
    '<span class="q3fichamaestitem">Artilharia ' + (maest.artilheiro || 0) + '</span>' +
    '<span class="q3fichamaestitem">Defesa ' + (maest.defesa || 0) + '</span>' +
    '<span class="q3fichamaestitem">Manobras ' + (maest.manobras || 0) + '</span>' +
    '<span class="q3fichamaestitem">Pomo ' + (maest.pomo || 0) + '</span>';

  var vass = slot.vassoura || { nome:'Vassoura Padrao de Hogwarts', bonus:0 };
